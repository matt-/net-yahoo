#!/usr/bin/perl

package Net::Yahoo::Constants;

use strict;
use warnings FATAL => 'all';
require Exporter;
our @ISA = qw(Exporter);
use vars qw($VERSION @ISA @EXPORT @EXPORT_OK %EXPORT_TAGS $AUTOLOAD);

our %EXPORT_TAGS = (
'service' => [ qw(
YAHOO_SERVICE_LOGON
YAHOO_SERVICE_LOGOFF
YAHOO_SERVICE_ISAWAY
YAHOO_SERVICE_ISBACK
YAHOO_SERVICE_IDLE
YAHOO_SERVICE_MESSAGE
YAHOO_SERVICE_IDACT
YAHOO_SERVICE_IDDEACT
YAHOO_SERVICE_MAILSTAT
YAHOO_SERVICE_USERSTAT
YAHOO_SERVICE_NEWMAIL
YAHOO_SERVICE_CHATINVITE
YAHOO_SERVICE_CALENDAR
YAHOO_SERVICE_NEWPERSONALMAIL
YAHOO_SERVICE_NEWCONTACT
YAHOO_SERVICE_ADDIDENT
YAHOO_SERVICE_ADDIGNORE
YAHOO_SERVICE_PING
YAHOO_SERVICE_GROUPRENAME
YAHOO_SERVICE_SYSMESSAGE
YAHOO_SERVICE_PASSTHROUGH2
YAHOO_SERVICE_CONFINVITE
YAHOO_SERVICE_CONFLOGON
YAHOO_SERVICE_CONFDECLINE
YAHOO_SERVICE_CONFLOGOFF
YAHOO_SERVICE_CONFADDINVITE
YAHOO_SERVICE_CONFMSG
YAHOO_SERVICE_CHATLOGON
YAHOO_SERVICE_CHATLOGOFF
YAHOO_SERVICE_CHATMSG
YAHOO_SERVICE_GAMELOGON
YAHOO_SERVICE_GAMELOGOFF
YAHOO_SERVICE_GAMEMSG
YAHOO_SERVICE_FILETRANSFER
YAHOO_SERVICE_VOICECHAT
YAHOO_SERVICE_NOTIFY
YAHOO_SERVICE_P2PFILEXFER
YAHOO_SERVICE_PEERTOPEER
YAHOO_SERVICE_AUTHRESP
YAHOO_SERVICE_LIST
YAHOO_SERVICE_AUTH
YAHOO_SERVICE_ADDBUDDY
YAHOO_SERVICE_REMBUDDY
YAHOO_SERVICE_IGNORECONTACT
YAHOO_SERVICE_REJECTCONTACT
YAHOO_SERVICE_PICTURE
) ],
'status' => [ qw(
YAHOO_STATUS_AVAILABLE
YAHOO_STATUS_BRB
YAHOO_STATUS_BUSY
YAHOO_STATUS_NOTATHOME
YAHOO_STATUS_NOTATDESK
YAHOO_STATUS_NOTINOFFICE
YAHOO_STATUS_ONPHONE
YAHOO_STATUS_ONVACATION
YAHOO_STATUS_OUTTOLUNCH
YAHOO_STATUS_STEPPEDOUT
YAHOO_STATUS_INVISIBLE
YAHOO_STATUS_CUSTOM
YAHOO_STATUS_IDLE
YAHOO_STATUS_OFFLINE
YAHOO_STATUS_TYPING
) ] );

our @EXPORT_OK = ( @{ $EXPORT_TAGS{'service'} }, @{ $EXPORT_TAGS{'status'} } );
our @EXPORT    = ( @{ $EXPORT_TAGS{'service'} }, @{ $EXPORT_TAGS{'status'} } );


# Set version of module
$VERSION = '0.01';

# Services
use constant YAHOO_SERVICE_LOGON            => 0x01;
use constant YAHOO_SERVICE_LOGOFF           => 0x02;
use constant YAHOO_SERVICE_ISAWAY           => 0x03;
use constant YAHOO_SERVICE_ISBACK           => 0x04;
use constant YAHOO_SERVICE_IDLE             => 0x05;
use constant YAHOO_SERVICE_MESSAGE          => 0x06;
use constant YAHOO_SERVICE_IDACT            => 0x07;
use constant YAHOO_SERVICE_IDDEACT          => 0x08;
use constant YAHOO_SERVICE_MAILSTAT         => 0x09;
use constant YAHOO_SERVICE_USERSTAT         => 0x0a;
use constant YAHOO_SERVICE_NEWMAIL          => 0x0b;
use constant YAHOO_SERVICE_CHATINVITE       => 0x0c;
use constant YAHOO_SERVICE_CALENDAR         => 0x0d;
use constant YAHOO_SERVICE_NEWPERSONALMAIL  => 0x0e;
use constant YAHOO_SERVICE_NEWCONTACT       => 0x0f;
use constant YAHOO_SERVICE_ADDIDENT         => 0x10;
use constant YAHOO_SERVICE_ADDIGNORE        => 0x11;
use constant YAHOO_SERVICE_PING             => 0x12;
use constant YAHOO_SERVICE_GROUPRENAME      => 0x13;
use constant YAHOO_SERVICE_SYSMESSAGE       => 0x14;
use constant YAHOO_SERVICE_PASSTHROUGH2     => 0x16;
use constant YAHOO_SERVICE_CONFINVITE       => 0x18;
use constant YAHOO_SERVICE_CONFLOGON        => 0x19;
use constant YAHOO_SERVICE_CONFDECLINE      => 0x1a;
use constant YAHOO_SERVICE_CONFLOGOFF       => 0x1b;
use constant YAHOO_SERVICE_CONFADDINVITE    => 0x1c;
use constant YAHOO_SERVICE_CONFMSG          => 0x1d;
use constant YAHOO_SERVICE_CHATLOGON        => 0x1e;
use constant YAHOO_SERVICE_CHATLOGOFF       => 0x1f;
use constant YAHOO_SERVICE_CHATMSG          => 0x20;
use constant YAHOO_SERVICE_GAMELOGON        => 0x28;
use constant YAHOO_SERVICE_GAMELOGOFF       => 0x28;
use constant YAHOO_SERVICE_GAMEMSG          => 0x2a;
use constant YAHOO_SERVICE_FILETRANSFER     => 0x46;
use constant YAHOO_SERVICE_VOICECHAT        => 0x4a;
use constant YAHOO_SERVICE_NOTIFY           => 0x4b;
use constant YAHOO_SERVICE_P2PFILEXFER      => 0x4d;
use constant YAHOO_SERVICE_PEERTOPEER       => 0x4f;
use constant YAHOO_SERVICE_AUTHRESP         => 0x54;
use constant YAHOO_SERVICE_LIST             => 0x55;
use constant YAHOO_SERVICE_AUTH             => 0x57;
use constant YAHOO_SERVICE_ADDBUDDY         => 0x83;
use constant YAHOO_SERVICE_REMBUDDY         => 0x84;
use constant YAHOO_SERVICE_IGNORECONTACT    => 0x85;
use constant YAHOO_SERVICE_REJECTCONTACT    => 0x86;
use constant YAHOO_SERVICE_PICTURE          => 0xbe;

# Status Codes
use constant YAHOO_STATUS_AVAILABLE    => 0;
use constant YAHOO_STATUS_BRB          => 1;
use constant YAHOO_STATUS_BUSY         => 2;
use constant YAHOO_STATUS_NOTATHOME    => 3;
use constant YAHOO_STATUS_NOTATDESK    => 4;
use constant YAHOO_STATUS_NOTINOFFICE  => 5;
use constant YAHOO_STATUS_ONPHONE      => 6;
use constant YAHOO_STATUS_ONVACATION   => 7;
use constant YAHOO_STATUS_OUTTOLUNCH   => 8;
use constant YAHOO_STATUS_STEPPEDOUT   => 9;
use constant YAHOO_STATUS_INVISIBLE    => 12;
use constant YAHOO_STATUS_CUSTOM       => 99;
use constant YAHOO_STATUS_IDLE         => 999;
use constant YAHOO_STATUS_OFFLINE      => 0x5a55aa56;
use constant YAHOO_STATUS_TYPING       => 0x16;

sub buildTables{
my $self = shift;
$self->{x_fn}[0] = [0x401117, 0x4016F6, 0x401AF7, 0x408F4B,  # 0
                    0x4095B6, 0x40999B, 0x40A006, 0x40CE09,  # 4
                    0x40CE91, 0x40E0BC, 0x40E144, 0x40F90B,  # 8
                    0x40F9C0, 0x410BD0, 0x410C58, 0x413D64,  # 12
                    0x414951, 0x40AD18, 0x40B383, 0x411B43,  # 16
                    0x4121AE, 0x4F1121, 0x4F178C, 0x4F201F,  # 20
                    0x4F268A, 0x4F0135, 0x4F07A0, 0x413C0D,  # 24
                    0x413CC2, 0x4147E7, 0x41489C, 0x416306,  # 28
                    0x41638E, 0x416ED1, 0x416F5F, 0x417E14,  # 32
                    0x417EC9, 0x4183F1, 0x418479, 0x419F64,  # 36
                    0x419FF2, 0x41A139, 0x41A7A4, 0x41AA13,  # 40
                    0x41AAC8, 0x41B3DE, 0x41BA49, 0x4F422E,  # 44
                    0x4F42BC, 0x4F5718, 0x4F57CD, 0x41CAFA,  # 48
                    0x41CB88, 0x41E838, 0x41E8C0, 0x4ED672,  # 52
                    0x4ED727, 0x421F25, 0x421FAD, 0x4FC502,  # 56
                    0x4FC58A, 0x4227C0, 0x422875, 0x422DAD,  # 60
                    0x422E62, 0x423203, 0x423291, 0x423782,  # 64
                    0x423837, 0x4251CA, 0x425258, 0x425603,  # 68
                    0x4256B8, 0x426D40, 0x426DF5, 0x427844,  # 72
                    0x427EAF, 0x428877, 0x4288FF, 0x4291B7,  # 76
                    0x429822, 0x42A19A, 0x42A24F, 0x42A489,  # 80
                    0x42AAF4, 0x42AD73, 0x42ADFB, 0x42B2DC,
                    0x42B364, 0x42B45C, 0x42B511, 0x42B7BB,
                    0x42BE26, 0x42C5A6, 0x42C634, 0x42D154];

$self->{x_fn}[1] = [0x42D7BF, 0x42D869, 0x42D8F1, 0x42DFD1,
                    0x42E63C, 0x42EE57, 0x42EF0C, 0x433193,
                    0x43321B, 0x434B9B, 0x434C29, 0x435EA5,
                    0x436510, 0x436CD1, 0x436D5F, 0x436E14,
                    0x436EA2, 0x437312, 0x4373C7, 0x437863,
                    0x437918, 0x4390DF, 0x43974A, 0x4EA6BD,
                    0x4EA772, 0x439D1C, 0x439DD1, 0x43BD01,
                    0x43C36C, 0x43E32E, 0x43E3E3, 0x43FD05,
                    0x43FDBA, 0x4424A6, 0x44255B, 0x442FD5,
                    0x44305D, 0x443762, 0x443817, 0x443FB8,
                    0x44406D, 0x445471, 0x445526, 0x44C4F8,
                    0x44C5AD, 0x44D0F1, 0x44D75C, 0x44F36E,
                    0x44F3FC, 0x44F746, 0x44F7CE, 0x44F949,
                    0x44F9D1, 0x45130F, 0x451397, 0x4538AE,
                    0x45393C, 0x453D17, 0x453E19, 0x4ED483,
                    0x4ED511, 0x45516E, 0x455223, 0x456B85,
                    0x456C0D, 0x4576FF, 0x457787, 0x459432,
                    0x4594BA, 0x459542, 0x4595F7, 0x45A272,
                    0x45A2FA, 0x45A732, 0x45A7C0, 0x45C521,
                    0x45CB8C, 0x45F3BE, 0x45F473, 0x46C119,
                    0x46C1A1, 0x46F65A, 0x46FCC5, 0x4709B7,
                    0x471022, 0x4F6E70, 0x4F6EFE, 0x4F6FE1,
                    0x4F764C, 0x4F7AF6, 0x4F7BAB, 0x4011A5,
                    0x401BAC, 0x40966B, 0x40A094, 0x40B5D3];

$self->{x_fn}[2] = [0x40CF1F, 0x40E1CC, 0x40FA48, 0x416416,
                    0x412263, 0x4F5ADA, 0x4F6652, 0x417004,
                    0x417F7E, 0x472255, 0x4722E7, 0x4726D7,
                    0x472796, 0x474544, 0x4745D6, 0x475502,
                    0x4755C1, 0x47A492, 0x47A551, 0x47B2C4,
                    0x47BE3C, 0x47C5E5, 0x47C6A4, 0x47CDFD,
                    0x47CE8F, 0x47D726, 0x47E29E, 0x47E753,
                    0x47E7EB, 0x47ECD0, 0x47ED68, 0x480895,
                    0x48092D, 0x481256, 0x481DCE, 0x481EBC,
                    0x481F7B, 0x483131, 0x4831F0, 0x484929,
                    0x4849C1, 0x41A859, 0x41AB56, 0x48A295,
                    0x48A32D, 0x492896, 0x492955, 0x4939BA,
                    0x494532, 0x49476D, 0x494805, 0x494C3E,
                    0x494CD6, 0x4978E5, 0x4979A4, 0x49898F,
                    0x498A4E, 0x49A264, 0x49A2FC, 0x49A590,
                    0x49B108, 0x49B296, 0x49B355, 0x49B530,
                    0x49B5C2, 0x49C0EA, 0x49C17C, 0x49EB02,
                    0x49F67A, 0x49F761, 0x49F7F9, 0x49FB5A,
                    0x49FBF2, 0x4A0507, 0x4A05C6, 0x4A254E,
                    0x4A30C6, 0x4A4D99, 0x4A4E31, 0x4A5192,
                    0x4A5D0A, 0x4A6546, 0x4A70BE, 0x4A7E6C,
                    0x4A7F2B, 0x4A837F, 0x4A8411, 0x4A88EB,
                    0x4A8983, 0x4A9638, 0x4A96D0, 0x4A9FDD,
                    0x4AA075, 0x4AE6C8, 0x4AF240, 0x4B1E08];

$self->{x_fn}[3] = [0x4B1EA0, 0x4B1FBD, 0x4B2055, 0x4B3830,
                    0x4B38C8, 0x4B3F23, 0x4B3FB5, 0x4B44E0,
                    0x4B459F, 0x4EB78B, 0x4EB81D, 0x4E6859,
                    0x4E6918, 0x4B74B2, 0x4B802A, 0x4B84A6,
                    0x4B8565, 0x4B94CA, 0x4B955C, 0x4B96E9,
                    0x4BA261, 0x4BA417, 0x4BA4AF, 0x4BB695,
                    0x4BB754, 0x4BB8EA, 0x4BB97C, 0x4EE9BE,
                    0x4EEA50, 0x4BBE02, 0x4BBE9A, 0x4FB7E6,
                    0x4FB8A5, 0x4E5A93, 0x4E5B52, 0x4BFB7A,
                    0x4BFC39, 0x4F324A, 0x4F32DC, 0x4C0861,
                    0x4C08F9, 0x4E99FC, 0x4E9ABB, 0x4E3B1E,
                    0x4E3BDD, 0x4E7B36, 0x4E7BC8, 0x4DFE2A,
                    0x4DFEE9, 0x4DDD87, 0x4DDE19, 0x4E4F50,
                    0x4E4FE2, 0x418AE4, 0x41A0A7, 0x4C2035,
                    0x4C20CD, 0x4DCEF4, 0x4DCF8C, 0x4C36CD,
                    0x4C3765, 0x4C4949, 0x4C49F0, 0x4C8D51,
                    0x4C98C9, 0x4CBF58, 0x4CBFEA, 0x4CC4E3,
                    0x4CC57B, 0x4CC88E, 0x4CD406, 0x4CEE8C,
                    0x4CEF24, 0x4D0BD6, 0x4D174E, 0x4D23E4,
                    0x4D247C, 0x4D2A11, 0x4D2AA9, 0x4CB803,
                    0x4CB8C2, 0x4D30B6, 0x4D3148, 0x4D3679,
                    0x4D3738, 0x4D40D7, 0x4D4169, 0x4D5E3D,
                    0x4D5EFC, 0x4D6DF0, 0x4D7968, 0x4D7FB7,
                    0x4D804F, 0x4DAA64, 0x4DAAFC, 0x4DB0E7];
# bitfield
$self->{fn}[0] = [0x408F4B, 0x40999B, 0x40AD18, 0x411B43,
                  0x4F1121, 0x4F201F, 0x4F0135, 0x418479,
                  0x41A139, 0x41B3DE, 0x4F42BC, 0x41E8C0,
                  0x4ED727, 0x427844, 0x427EAF, 0x4288FF,
                  0x4291B7, 0x42A489, 0x42B7BB, 0x42C634,
                  0x42D154, 0x42D8F1, 0x42DFD1, 0x435EA5,
                  0x436510, 0x4390DF, 0x439DD1, 0x43BD01,
                  0x43C36C, 0x44255B, 0x44D0F1, 0x44D75C,
                  0x44F9D1, 0x453E19, 0x455223, 0x456C0D,
                  0x4595F7, 0x45A7C0, 0x45C521, 0x45CB8C,
                  0x46C1A1, 0x46F65A, 0x4709B7, 0x4F6FE1,
                  0x401BAC, 0x40B5D3, 0x40CF1F, 0x4F5ADA,
                  0x472796, 0x4745D6, 0x47B2C4, 0x47D726,
                  0x481256, 0x481F7B, 0x4831F0, 0x492955,
                  0x4939BA, 0x4979A4, 0x498A4E, 0x49A590,
                  0x49EB02, 0x4A254E, 0x4A5192, 0x4A6546,
                  0x4A70BE, 0x4AE6C8, 0x4B2055, 0x4EB81D,
                  0x4B74B2, 0x4B96E9, 0x4BA4AF, 0x4BBE9A,
                  0x4F32DC, 0x4C08F9, 0x4C8D51, 0x4C98C9,
                  0x4CC88E, 0x4CD406, 0x4CEF24, 0x4D0BD6,
                  0x4D5EFC, 0x4D6DF0, 0x4DB0E7];

# lookup
$self->{fn}[1] = [0x4016F6, 0x401AF7, 0x4095B6, 0x40F90B,
                  0x40B383, 0x4121AE, 0x4F178C, 0x4F268A,
                  0x413C0D, 0x4147E7, 0x41489C, 0x417E14,
                  0x417EC9, 0x419FF2, 0x41A7A4, 0x41AA13,
                  0x4F5718, 0x4ED672, 0x4227C0, 0x422875,
                  0x422DAD, 0x423782, 0x425258, 0x425603,
                  0x426D40, 0x426DF5, 0x42A19A, 0x42AAF4,
                  0x42B45C, 0x42E63C, 0x42EE57, 0x436D5F,
                  0x437312, 0x437863, 0x437918, 0x43974A,
                  0x4EA6BD, 0x439D1C, 0x43E32E, 0x43FD05,
                  0x43FDBA, 0x4424A6, 0x443762, 0x443817,
                  0x443FB8, 0x445471, 0x44C4F8, 0x453D17,
                  0x45516E, 0x459542, 0x45F3BE, 0x46FCC5,
                  0x4F764C, 0x4F7AF6, 0x4011A5, 0x40966B,
                  0x4F6652, 0x4726D7, 0x475502, 0x4755C1,
                  0x47A492, 0x47C5E5, 0x47C6A4, 0x47E7EB,
                  0x481DCE, 0x481EBC, 0x483131, 0x41AB56,
                  0x48A32D, 0x492896, 0x4978E5, 0x49898F,
                  0x49B296, 0x49B355, 0x49F67A, 0x4A0507,
                  0x4A7E6C, 0x4A8411, 0x4B38C8, 0x4B44E0,
                  0x4E6859, 0x4B84A6, 0x4B955C, 0x4BB695,
                  0x4BB97C, 0x4EEA50, 0x4FB7E6, 0x4E5A93,
                  0x4E5B52, 0x4BFB7A, 0x4BFC39, 0x4E99FC,
                  0x4E9ABB, 0x4E3B1E, 0x4DFE2A, 0x4DDE19,
                  0x4DCF8C, 0x4D247C, 0x4CB803, 0x4D3679,
                  0x4D3738, 0x4D4169, 0x4D5E3D, 0x4D7968,
                  0x4D804F];

# muladd
$self->{fn}[2] = [0x401117, 0x40A006, 0x40CE91, 0x410C58,
                  0x414951, 0x4F07A0, 0x416ED1, 0x416F5F,
                  0x419F64, 0x41AAC8, 0x4F422E, 0x41CAFA,
                  0x41CB88, 0x421FAD, 0x4FC58A, 0x422E62,
                  0x423203, 0x4251CA, 0x429822, 0x42ADFB,
                  0x42B511, 0x42C5A6, 0x42D7BF, 0x42EF0C,
                  0x43321B, 0x434B9B, 0x436CD1, 0x436E14,
                  0x44305D, 0x44406D, 0x445526, 0x44F36E,
                  0x4538AE, 0x4ED483, 0x45A2FA, 0x45A732,
                  0x471022, 0x4F6E70, 0x4F6EFE, 0x40E1CC,
                  0x416416, 0x417004, 0x4722E7, 0x47BE3C,
                  0x47CE8F, 0x47E29E, 0x47E753, 0x47ECD0,
                  0x480895, 0x48092D, 0x484929, 0x48A295,
                  0x49476D, 0x494805, 0x494C3E, 0x49A264,
                  0x49A2FC, 0x49B5C2, 0x49F761, 0x49F7F9,
                  0x49FB5A, 0x49FBF2, 0x4A05C6, 0x4A4D99,
                  0x4A7F2B, 0x4A88EB, 0x4A9638, 0x4A96D0,
                  0x4A9FDD, 0x4AA075, 0x4AF240, 0x4B1E08,
                  0x4B1EA0, 0x4B1FBD, 0x4B3830, 0x4B3FB5,
                  0x4B459F, 0x4E6918, 0x4B8565, 0x4BA417,
                  0x4BBE02, 0x4C0861, 0x4E3BDD, 0x4E7BC8,
                  0x4DFEE9, 0x418AE4, 0x4C2035, 0x4DCEF4,
                  0x4C36CD, 0x4C4949, 0x4CBFEA, 0x4CC4E3,
                  0x4CC57B, 0x4CEE8C, 0x4D23E4, 0x4D2A11,
                  0x4D2AA9, 0x4CB8C2, 0x4D3148, 0x4D7FB7,
                  0x4DAA64, 0x4DAAFC];

# xor
$self->{fn}[3] = [0x40CE09, 0x40E0BC, 0x40E144, 0x40F9C0,
                  0x410BD0, 0x413D64, 0x413CC2, 0x416306,
                  0x41638E, 0x4183F1, 0x41BA49, 0x4F57CD,
                  0x41E838, 0x421F25, 0x4FC502, 0x423291,
                  0x423837, 0x4256B8, 0x428877, 0x42A24F,
                  0x42AD73, 0x42B2DC, 0x42B364, 0x42BE26,
                  0x42D869, 0x433193, 0x434C29, 0x436EA2,
                  0x4373C7, 0x4EA772, 0x43E3E3, 0x442FD5,
                  0x44C5AD, 0x44F3FC, 0x44F746, 0x44F7CE,
                  0x44F949, 0x45130F, 0x451397, 0x45393C,
                  0x4ED511, 0x456B85, 0x4576FF, 0x457787,
                  0x459432, 0x4594BA, 0x45A272, 0x45F473,
                  0x46C119, 0x4F7BAB, 0x40A094, 0x40FA48,
                  0x412263, 0x417F7E, 0x472255, 0x474544,
                  0x47A551, 0x47CDFD, 0x47ED68, 0x4849C1,
                  0x41A859, 0x494532, 0x494CD6, 0x49B108,
                  0x49B530, 0x49C0EA, 0x49C17C, 0x4A30C6,
                  0x4A4E31, 0x4A5D0A, 0x4A837F, 0x4A8983,
                  0x4B3F23, 0x4EB78B, 0x4B802A, 0x4B94CA,
                  0x4BA261, 0x4BB754, 0x4BB8EA, 0x4EE9BE,
                  0x4FB8A5, 0x4F324A, 0x4E7B36, 0x4DDD87,
                  0x4E4F50, 0x4E4FE2, 0x41A0A7, 0x4C20CD,
                  0x4C3765, 0x4C49F0, 0x4CBF58, 0x4D174E,
                  0x4D30B6, 0x4D40D7];

# build fntype hash
for(0..3){
    foreach my $fnvalue (@{$self->{fn}[$_]}) {
        $self->{fntype}->{$fnvalue} = $_;
    }
}

$self->{fn_unk_bitfield}->{0x408F4B} = 0x55F320;
$self->{fn_unk_bitfield}->{0x40999B} = 0x55F340;
$self->{fn_unk_bitfield}->{0x40AD18} = 0x55F360;
$self->{fn_unk_bitfield}->{0x411B43} = 0x55F604;
$self->{fn_unk_bitfield}->{0x4F1121} = 0x5628C4;
$self->{fn_unk_bitfield}->{0x4F201F} = 0x5628E4;
$self->{fn_unk_bitfield}->{0x4F0135} = 0x5628A4;
$self->{fn_unk_bitfield}->{0x418479} = 0x55F6A4;
$self->{fn_unk_bitfield}->{0x41A139} = 0x55F6C4;
$self->{fn_unk_bitfield}->{0x41B3DE} = 0x55F6E4;
$self->{fn_unk_bitfield}->{0x4F42BC} = 0x562928;
$self->{fn_unk_bitfield}->{0x41E8C0} = 0x55F744;
$self->{fn_unk_bitfield}->{0x4ED727} = 0x562884;
$self->{fn_unk_bitfield}->{0x427844} = 0x55F784;
$self->{fn_unk_bitfield}->{0x427EAF} = 0x55F7A4;
$self->{fn_unk_bitfield}->{0x4288FF} = 0x55F7C4;
$self->{fn_unk_bitfield}->{0x4291B7} = 0x55F7E4;
$self->{fn_unk_bitfield}->{0x42A489} = 0x55F804;
$self->{fn_unk_bitfield}->{0x42B7BB} = 0x55F824;
$self->{fn_unk_bitfield}->{0x42C634} = 0x55F844;
$self->{fn_unk_bitfield}->{0x42D154} = 0x55F864;
$self->{fn_unk_bitfield}->{0x42D8F1} = 0x55F884;
$self->{fn_unk_bitfield}->{0x42DFD1} = 0x55F8A4;
$self->{fn_unk_bitfield}->{0x435EA5} = 0x55F91C;
$self->{fn_unk_bitfield}->{0x436510} = 0x55F93C;
$self->{fn_unk_bitfield}->{0x4390DF} = 0x55F9D0;
$self->{fn_unk_bitfield}->{0x439DD1} = 0x560280;
$self->{fn_unk_bitfield}->{0x43BD01} = 0x56034C;
$self->{fn_unk_bitfield}->{0x43C36C} = 0x56036C;
$self->{fn_unk_bitfield}->{0x44255B} = 0x5605B0;
$self->{fn_unk_bitfield}->{0x44D0F1} = 0x560BCC;
$self->{fn_unk_bitfield}->{0x44D75C} = 0x560BEC;
$self->{fn_unk_bitfield}->{0x44F9D1} = 0x560C10;
$self->{fn_unk_bitfield}->{0x453E19} = 0x560D34;
$self->{fn_unk_bitfield}->{0x455223} = 0x560E38;
$self->{fn_unk_bitfield}->{0x456C0D} = 0x561014;
$self->{fn_unk_bitfield}->{0x4595F7} = 0x561074;
$self->{fn_unk_bitfield}->{0x45A7C0} = 0x561094;
$self->{fn_unk_bitfield}->{0x45C521} = 0x5610B4;
$self->{fn_unk_bitfield}->{0x45CB8C} = 0x5610D4;
$self->{fn_unk_bitfield}->{0x46C1A1} = 0x561190;
$self->{fn_unk_bitfield}->{0x46F65A} = 0x5611B0;
$self->{fn_unk_bitfield}->{0x4709B7} = 0x5611D0;
$self->{fn_unk_bitfield}->{0x4F6FE1} = 0x562968;
$self->{fn_unk_bitfield}->{0x401BAC} = 0x55F214;
$self->{fn_unk_bitfield}->{0x40B5D3} = 0x55F380;
$self->{fn_unk_bitfield}->{0x40CF1F} = 0x55F3F8;
$self->{fn_unk_bitfield}->{0x4F5ADA} = 0x562948;
$self->{fn_unk_bitfield}->{0x472796} = 0x561258;
$self->{fn_unk_bitfield}->{0x4745D6} = 0x561278;
$self->{fn_unk_bitfield}->{0x47B2C4} = 0x561298;
$self->{fn_unk_bitfield}->{0x47D726} = 0x5613A0;
$self->{fn_unk_bitfield}->{0x481256} = 0x5613C0;
$self->{fn_unk_bitfield}->{0x481F7B} = 0x5613E4;
$self->{fn_unk_bitfield}->{0x4831F0} = 0x561404;
$self->{fn_unk_bitfield}->{0x492955} = 0x5615A0;
$self->{fn_unk_bitfield}->{0x4939BA} = 0x561628;
$self->{fn_unk_bitfield}->{0x4979A4} = 0x561748;
$self->{fn_unk_bitfield}->{0x498A4E} = 0x561768;
$self->{fn_unk_bitfield}->{0x49A590} = 0x5617F0;
$self->{fn_unk_bitfield}->{0x49EB02} = 0x561EA0;
$self->{fn_unk_bitfield}->{0x4A254E} = 0x561F38;
$self->{fn_unk_bitfield}->{0x4A5192} = 0x561F58;
$self->{fn_unk_bitfield}->{0x4A6546} = 0x561F7C;
$self->{fn_unk_bitfield}->{0x4A70BE} = 0x561F9C;
$self->{fn_unk_bitfield}->{0x4AE6C8} = 0x562198;
$self->{fn_unk_bitfield}->{0x4B2055} = 0x5622F0;
$self->{fn_unk_bitfield}->{0x4EB81D} = 0x562864;
$self->{fn_unk_bitfield}->{0x4B74B2} = 0x56238C;
$self->{fn_unk_bitfield}->{0x4B96E9} = 0x5623AC;
$self->{fn_unk_bitfield}->{0x4BA4AF} = 0x5623CC;
$self->{fn_unk_bitfield}->{0x4BBE9A} = 0x5623EC;
$self->{fn_unk_bitfield}->{0x4F32DC} = 0x562908;
$self->{fn_unk_bitfield}->{0x4C08F9} = 0x56249C;
$self->{fn_unk_bitfield}->{0x4C8D51} = 0x562544;
$self->{fn_unk_bitfield}->{0x4C98C9} = 0x562564;
$self->{fn_unk_bitfield}->{0x4CC88E} = 0x5625A0;
$self->{fn_unk_bitfield}->{0x4CD406} = 0x5625C0;
$self->{fn_unk_bitfield}->{0x4CEF24} = 0x5625E0;
$self->{fn_unk_bitfield}->{0x4D0BD6} = 0x562600;
$self->{fn_unk_bitfield}->{0x4D5EFC} = 0x562620;
$self->{fn_unk_bitfield}->{0x4D6DF0} = 0x562640;
$self->{fn_unk_bitfield}->{0x4DB0E7} = 0x562660;

# bitfield
$self->{unk_bitfield}->{0x55F320} = [
        0x19, 0x05, 0x09, 0x1C, 0x0B, 0x1A, 0x12, 0x03,
        0x06, 0x04, 0x0D, 0x1D, 0x15, 0x0E, 0x1B, 0x18,
        0x00, 0x07, 0x08, 0x02, 0x13, 0x1F, 0x0C, 0x1E,
        0x16, 0x0A, 0x10, 0x0F, 0x01, 0x14, 0x11, 0x17];

$self->{unk_bitfield}->{0x55F340} = [
        0x1F, 0x0B, 0x00, 0x1E, 0x03, 0x0E, 0x15, 0x01,
        0x1A, 0x17, 0x1D, 0x1B, 0x11, 0x0F, 0x0A, 0x12,
        0x13, 0x18, 0x02, 0x04, 0x09, 0x06, 0x0D, 0x07,
        0x08, 0x05, 0x10, 0x19, 0x0C, 0x14, 0x16, 0x1C];

$self->{unk_bitfield}->{0x55F360} = [
        0x1A, 0x1C, 0x0F, 0x0C, 0x00, 0x02, 0x13, 0x09,
        0x11, 0x05, 0x0D, 0x12, 0x18, 0x0B, 0x04, 0x10,
        0x14, 0x1B, 0x1E, 0x16, 0x07, 0x08, 0x03, 0x17,
        0x19, 0x1F, 0x01, 0x0E, 0x15, 0x06, 0x0A, 0x1D];

$self->{unk_bitfield}->{0x55F604} = [
        0x13, 0x08, 0x1E, 0x1D, 0x17, 0x16, 0x07, 0x1F,
        0x0E, 0x03, 0x1A, 0x19, 0x01, 0x12, 0x11, 0x10,
        0x09, 0x0C, 0x0F, 0x14, 0x0B, 0x05, 0x00, 0x04,
        0x1C, 0x18, 0x0A, 0x15, 0x02, 0x1B, 0x06, 0x0D];

$self->{unk_bitfield}->{0x5628C4} = [
        0x1D, 0x12, 0x11, 0x0D, 0x1E, 0x19, 0x16, 0x1B,
        0x18, 0x13, 0x07, 0x17, 0x0C, 0x02, 0x00, 0x15,
        0x0E, 0x08, 0x05, 0x01, 0x10, 0x06, 0x04, 0x0F,
        0x1F, 0x1A, 0x0B, 0x09, 0x0A, 0x14, 0x1C, 0x03];

$self->{unk_bitfield}->{0x5628E4} = [
        0x14, 0x1B, 0x18, 0x00, 0x1F, 0x15, 0x17, 0x07,
        0x11, 0x1A, 0x0E, 0x13, 0x12, 0x06, 0x01, 0x03,
        0x1C, 0x0C, 0x0B, 0x1D, 0x10, 0x0F, 0x09, 0x19,
        0x0D, 0x1E, 0x04, 0x05, 0x08, 0x16, 0x0A, 0x02];

$self->{unk_bitfield}->{0x5628A4} = [
        0x04, 0x14, 0x13, 0x15, 0x1A, 0x1B, 0x0F, 0x16,
        0x02, 0x0D, 0x0C, 0x06, 0x10, 0x17, 0x01, 0x0B,
        0x1E, 0x08, 0x1C, 0x18, 0x19, 0x0A, 0x1F, 0x05,
        0x11, 0x09, 0x1D, 0x07, 0x0E, 0x12, 0x03, 0x00];

$self->{unk_bitfield}->{0x55F6A4} = [
        0x0D, 0x0B, 0x11, 0x02, 0x05, 0x1B, 0x08, 0x1D,
        0x04, 0x14, 0x01, 0x09, 0x00, 0x19, 0x1E, 0x15,
        0x1F, 0x0A, 0x0F, 0x1C, 0x10, 0x16, 0x0C, 0x07,
        0x13, 0x1A, 0x06, 0x17, 0x0E, 0x12, 0x18, 0x03];

$self->{unk_bitfield}->{0x55F6C4} = [
        0x0B, 0x15, 0x1C, 0x0C, 0x06, 0x0A, 0x1D, 0x16,
        0x12, 0x0E, 0x04, 0x11, 0x1F, 0x0F, 0x07, 0x02,
        0x17, 0x13, 0x19, 0x18, 0x0D, 0x10, 0x1A, 0x05,
        0x03, 0x00, 0x01, 0x08, 0x09, 0x14, 0x1B, 0x1E];

$self->{unk_bitfield}->{0x55F6E4} = [
        0x0A, 0x11, 0x17, 0x03, 0x05, 0x0B, 0x18, 0x13,
        0x09, 0x02, 0x00, 0x1C, 0x0C, 0x08, 0x1B, 0x14,
        0x06, 0x0E, 0x01, 0x0D, 0x16, 0x1E, 0x1D, 0x19,
        0x0F, 0x1A, 0x10, 0x04, 0x12, 0x15, 0x07, 0x1F];

$self->{unk_bitfield}->{0x562928} = [
        0x19, 0x13, 0x1B, 0x01, 0x1C, 0x0D, 0x0C, 0x15,
        0x0B, 0x00, 0x1A, 0x0F, 0x12, 0x16, 0x08, 0x0A,
        0x03, 0x06, 0x14, 0x10, 0x18, 0x04, 0x11, 0x1D,
        0x1F, 0x07, 0x17, 0x05, 0x02, 0x0E, 0x1E, 0x09];

$self->{unk_bitfield}->{0x55F744} = [
        0x15, 0x05, 0x08, 0x19, 0x02, 0x18, 0x1E, 0x07,
        0x0D, 0x0C, 0x1A, 0x06, 0x17, 0x03, 0x10, 0x09,
        0x01, 0x11, 0x1C, 0x04, 0x0F, 0x1F, 0x12, 0x0B,
        0x1B, 0x13, 0x0A, 0x16, 0x0E, 0x00, 0x1D, 0x14];

$self->{unk_bitfield}->{0x562884} = [
        0x00, 0x1C, 0x0E, 0x0C, 0x06, 0x16, 0x09, 0x12,
        0x01, 0x13, 0x0B, 0x14, 0x11, 0x08, 0x04, 0x18,
        0x10, 0x1B, 0x15, 0x03, 0x02, 0x19, 0x1A, 0x17,
        0x1E, 0x1F, 0x0F, 0x07, 0x0D, 0x05, 0x1D, 0x0A];

$self->{unk_bitfield}->{0x55F784} = [
        0x12, 0x18, 0x0E, 0x08, 0x16, 0x05, 0x06, 0x00,
        0x11, 0x17, 0x15, 0x1B, 0x14, 0x01, 0x1F, 0x19,
        0x04, 0x0D, 0x0A, 0x0F, 0x10, 0x07, 0x1D, 0x03,
        0x0B, 0x13, 0x0C, 0x09, 0x1E, 0x02, 0x1A, 0x1C];

$self->{unk_bitfield}->{0x55F7A4} = [
        0x16, 0x02, 0x06, 0x0E, 0x0D, 0x1C, 0x08, 0x0A,
        0x0F, 0x13, 0x0B, 0x18, 0x07, 0x04, 0x14, 0x01,
        0x1B, 0x05, 0x17, 0x1E, 0x11, 0x1A, 0x10, 0x1F,
        0x12, 0x19, 0x1D, 0x03, 0x0C, 0x00, 0x09, 0x15];

$self->{unk_bitfield}->{0x55F7C4} = [
        0x13, 0x18, 0x04, 0x1F, 0x1D, 0x11, 0x03, 0x00,
        0x10, 0x12, 0x06, 0x0A, 0x1C, 0x07, 0x15, 0x0E,
        0x08, 0x05, 0x0C, 0x09, 0x01, 0x02, 0x16, 0x0B,
        0x1A, 0x17, 0x14, 0x1E, 0x0D, 0x0F, 0x19, 0x1B];

$self->{unk_bitfield}->{0x55F7E4} = [
        0x00, 0x08, 0x15, 0x1D, 0x05, 0x18, 0x06, 0x07,
        0x1F, 0x01, 0x0B, 0x03, 0x19, 0x13, 0x02, 0x1C,
        0x17, 0x11, 0x0E, 0x1E, 0x0C, 0x0F, 0x09, 0x1A,
        0x1B, 0x16, 0x10, 0x0D, 0x0A, 0x14, 0x12, 0x04];

$self->{unk_bitfield}->{0x55F804} = [
        0x1A, 0x0E, 0x0A, 0x17, 0x1F, 0x08, 0x10, 0x14,
        0x0C, 0x0F, 0x09, 0x1C, 0x06, 0x18, 0x1E, 0x12,
        0x15, 0x00, 0x11, 0x13, 0x0D, 0x01, 0x0B, 0x03,
        0x16, 0x19, 0x05, 0x1D, 0x02, 0x07, 0x04, 0x1B];

$self->{unk_bitfield}->{0x55F824} = [
        0x18, 0x1D, 0x16, 0x10, 0x11, 0x04, 0x1E, 0x08,
        0x19, 0x0E, 0x0F, 0x02, 0x14, 0x1C, 0x07, 0x17,
        0x0D, 0x09, 0x12, 0x1A, 0x05, 0x01, 0x0B, 0x0A,
        0x13, 0x15, 0x0C, 0x00, 0x06, 0x1F, 0x03, 0x1B];

$self->{unk_bitfield}->{0x55F844} = [
        0x13, 0x08, 0x15, 0x01, 0x17, 0x10, 0x0F, 0x1F,
        0x1D, 0x0D, 0x12, 0x03, 0x06, 0x0A, 0x1C, 0x19,
        0x1A, 0x04, 0x1B, 0x02, 0x16, 0x1E, 0x11, 0x00,
        0x14, 0x09, 0x0C, 0x18, 0x05, 0x07, 0x0E, 0x0B];

$self->{unk_bitfield}->{0x55F864} = [
        0x1F, 0x0F, 0x19, 0x07, 0x18, 0x05, 0x1E, 0x1D,
        0x15, 0x08, 0x17, 0x10, 0x0A, 0x0E, 0x0C, 0x1B,
        0x02, 0x13, 0x03, 0x0D, 0x04, 0x1A, 0x06, 0x09,
        0x12, 0x1C, 0x0B, 0x16, 0x14, 0x01, 0x11, 0x00];

$self->{unk_bitfield}->{0x55F884} = [
        0x16, 0x18, 0x1C, 0x0E, 0x12, 0x00, 0x04, 0x1B,
        0x1F, 0x13, 0x17, 0x0A, 0x1E, 0x03, 0x0C, 0x01,
        0x0F, 0x10, 0x02, 0x08, 0x14, 0x09, 0x19, 0x15,
        0x06, 0x0D, 0x0B, 0x1D, 0x05, 0x07, 0x11, 0x1A];

$self->{unk_bitfield}->{0x55F8A4} = [
        0x1C, 0x0D, 0x1B, 0x07, 0x17, 0x0E, 0x06, 0x01,
        0x12, 0x19, 0x03, 0x0B, 0x10, 0x08, 0x00, 0x1E,
        0x0A, 0x04, 0x1A, 0x1D, 0x0C, 0x18, 0x02, 0x13,
        0x0F, 0x11, 0x05, 0x09, 0x15, 0x16, 0x1F, 0x14];

$self->{unk_bitfield}->{0x55F91C} = [
        0x01, 0x02, 0x1D, 0x10, 0x0E, 0x11, 0x08, 0x14,
        0x12, 0x09, 0x15, 0x17, 0x16, 0x04, 0x06, 0x1B,
        0x07, 0x1A, 0x18, 0x13, 0x0A, 0x1E, 0x1C, 0x1F,
        0x0C, 0x0B, 0x0D, 0x05, 0x0F, 0x00, 0x19, 0x03];

$self->{unk_bitfield}->{0x55F93C} = [
        0x01, 0x12, 0x13, 0x09, 0x0B, 0x19, 0x03, 0x0E,
        0x02, 0x1F, 0x1D, 0x1B, 0x1E, 0x11, 0x06, 0x05,
        0x00, 0x16, 0x07, 0x0C, 0x15, 0x0D, 0x1A, 0x08,
        0x18, 0x10, 0x0F, 0x17, 0x1C, 0x0A, 0x04, 0x14];

$self->{unk_bitfield}->{0x55F9D0} = [
        0x1C, 0x06, 0x1E, 0x10, 0x1D, 0x05, 0x00, 0x0E,
        0x0C, 0x02, 0x11, 0x19, 0x15, 0x18, 0x16, 0x07,
        0x1F, 0x0B, 0x14, 0x01, 0x0F, 0x09, 0x0D, 0x13,
        0x03, 0x08, 0x12, 0x04, 0x1B, 0x0A, 0x17, 0x1A];

$self->{unk_bitfield}->{0x560280} = [
        0x03, 0x05, 0x0D, 0x09, 0x1A, 0x16, 0x08, 0x10,
        0x06, 0x1E, 0x1C, 0x15, 0x02, 0x04, 0x17, 0x0C,
        0x18, 0x0B, 0x19, 0x11, 0x1B, 0x14, 0x13, 0x0A,
        0x0E, 0x00, 0x1D, 0x1F, 0x01, 0x0F, 0x07, 0x12];

$self->{unk_bitfield}->{0x56034C} = [
        0x01, 0x0A, 0x1E, 0x14, 0x10, 0x1D, 0x0D, 0x17,
        0x0E, 0x0C, 0x0F, 0x12, 0x04, 0x1A, 0x05, 0x02,
        0x08, 0x1C, 0x09, 0x1F, 0x0B, 0x13, 0x19, 0x1B,
        0x11, 0x00, 0x16, 0x06, 0x03, 0x18, 0x15, 0x07];

$self->{unk_bitfield}->{0x56036C} = [
        0x1C, 0x18, 0x0C, 0x09, 0x05, 0x03, 0x15, 0x12,
        0x0D, 0x02, 0x08, 0x0E, 0x19, 0x07, 0x13, 0x17,
        0x1E, 0x1D, 0x1F, 0x11, 0x06, 0x0A, 0x0B, 0x14,
        0x0F, 0x10, 0x01, 0x1B, 0x00, 0x04, 0x1A, 0x16];

$self->{unk_bitfield}->{0x5605B0} = [
        0x17, 0x13, 0x0E, 0x1A, 0x0D, 0x18, 0x19, 0x10,
        0x14, 0x11, 0x16, 0x05, 0x04, 0x00, 0x12, 0x0A,
        0x02, 0x07, 0x03, 0x0B, 0x09, 0x1F, 0x1C, 0x0F,
        0x0C, 0x06, 0x1B, 0x08, 0x1D, 0x01, 0x15, 0x1E];

$self->{unk_bitfield}->{0x560BCC} = [
        0x1C, 0x0D, 0x1E, 0x01, 0x06, 0x16, 0x18, 0x17,
        0x0B, 0x1F, 0x04, 0x0F, 0x00, 0x19, 0x08, 0x0A,
        0x11, 0x03, 0x05, 0x07, 0x09, 0x0C, 0x15, 0x14,
        0x1A, 0x12, 0x13, 0x0E, 0x1D, 0x10, 0x02, 0x1B];

$self->{unk_bitfield}->{0x560BEC} = [
        0x0E, 0x02, 0x17, 0x12, 0x1E, 0x09, 0x15, 0x03,
        0x01, 0x0B, 0x0F, 0x11, 0x10, 0x0A, 0x16, 0x06,
        0x07, 0x00, 0x1C, 0x1D, 0x1F, 0x0C, 0x18, 0x04,
        0x13, 0x0D, 0x1B, 0x08, 0x19, 0x14, 0x05, 0x1A];

$self->{unk_bitfield}->{0x560C10} = [
        0x12, 0x0B, 0x11, 0x01, 0x07, 0x0E, 0x1A, 0x0D,
        0x1E, 0x18, 0x14, 0x1F, 0x0A, 0x17, 0x19, 0x1B,
        0x00, 0x10, 0x0C, 0x08, 0x13, 0x02, 0x0F, 0x1D,
        0x09, 0x06, 0x04, 0x16, 0x15, 0x1C, 0x05, 0x03];

$self->{unk_bitfield}->{0x560D34} = [
        0x03, 0x02, 0x1D, 0x0E, 0x09, 0x1A, 0x0C, 0x11,
        0x1C, 0x0D, 0x08, 0x12, 0x19, 0x10, 0x04, 0x17,
        0x15, 0x05, 0x0A, 0x00, 0x13, 0x16, 0x1B, 0x18,
        0x1E, 0x0B, 0x0F, 0x01, 0x07, 0x14, 0x1F, 0x06];

$self->{unk_bitfield}->{0x560E38} = [
        0x1B, 0x0A, 0x1F, 0x01, 0x10, 0x08, 0x0E, 0x18,
        0x06, 0x04, 0x00, 0x1C, 0x0C, 0x19, 0x0D, 0x16,
        0x02, 0x03, 0x09, 0x07, 0x13, 0x0F, 0x05, 0x12,
        0x17, 0x1E, 0x1A, 0x1D, 0x0B, 0x11, 0x14, 0x15];

$self->{unk_bitfield}->{0x561014} = [
        0x02, 0x1A, 0x0D, 0x15, 0x01, 0x16, 0x1E, 0x00,
        0x08, 0x1B, 0x04, 0x10, 0x1C, 0x18, 0x19, 0x14,
        0x0C, 0x11, 0x0B, 0x0E, 0x03, 0x0A, 0x07, 0x12,
        0x1D, 0x17, 0x13, 0x06, 0x0F, 0x05, 0x09, 0x1F];

$self->{unk_bitfield}->{0x561074} = [
        0x11, 0x07, 0x0F, 0x0A, 0x19, 0x1D, 0x0B, 0x09,
        0x1C, 0x1E, 0x14, 0x06, 0x0C, 0x16, 0x13, 0x04,
        0x15, 0x18, 0x00, 0x0D, 0x12, 0x05, 0x08, 0x02,
        0x10, 0x1A, 0x1F, 0x01, 0x17, 0x0E, 0x03, 0x1B];

$self->{unk_bitfield}->{0x561094} = [
        0x17, 0x0E, 0x1D, 0x13, 0x0B, 0x19, 0x03, 0x06,
        0x09, 0x01, 0x0D, 0x15, 0x1C, 0x16, 0x18, 0x1B,
        0x11, 0x10, 0x00, 0x1E, 0x1F, 0x08, 0x12, 0x0F,
        0x02, 0x04, 0x07, 0x1A, 0x14, 0x0A, 0x0C, 0x05];

$self->{unk_bitfield}->{0x5610B4} = [
        0x09, 0x08, 0x17, 0x10, 0x0A, 0x07, 0x1C, 0x1F,
        0x04, 0x0E, 0x01, 0x0C, 0x0D, 0x1B, 0x03, 0x15,
        0x02, 0x1E, 0x18, 0x19, 0x0F, 0x06, 0x1A, 0x0B,
        0x05, 0x11, 0x14, 0x00, 0x16, 0x1D, 0x12, 0x13];

$self->{unk_bitfield}->{0x5610D4} = [
        0x15, 0x1C, 0x1D, 0x14, 0x0F, 0x1A, 0x05, 0x02,
        0x07, 0x09, 0x06, 0x08, 0x1F, 0x00, 0x10, 0x13,
        0x0D, 0x03, 0x0C, 0x18, 0x0E, 0x16, 0x1B, 0x1E,
        0x12, 0x04, 0x11, 0x0A, 0x01, 0x0B, 0x17, 0x19];

$self->{unk_bitfield}->{0x561190} = [
        0x03, 0x04, 0x0C, 0x18, 0x10, 0x0D, 0x13, 0x1B,
        0x1F, 0x07, 0x11, 0x17, 0x1C, 0x1D, 0x05, 0x06,
        0x0A, 0x12, 0x02, 0x1A, 0x0B, 0x01, 0x0E, 0x08,
        0x14, 0x16, 0x00, 0x15, 0x19, 0x09, 0x0F, 0x1E];

$self->{unk_bitfield}->{0x5611B0} = [
        0x1E, 0x10, 0x01, 0x07, 0x11, 0x16, 0x15, 0x17,
        0x1F, 0x14, 0x0C, 0x1C, 0x06, 0x03, 0x00, 0x18,
        0x08, 0x0E, 0x02, 0x1B, 0x09, 0x0D, 0x19, 0x05,
        0x0F, 0x12, 0x0B, 0x13, 0x0A, 0x04, 0x1D, 0x1A];

$self->{unk_bitfield}->{0x5611D0} = [
        0x1C, 0x07, 0x05, 0x1A, 0x10, 0x1D, 0x14, 0x12,
        0x08, 0x0F, 0x0C, 0x01, 0x04, 0x1B, 0x16, 0x0A,
        0x11, 0x02, 0x1F, 0x13, 0x0D, 0x1E, 0x17, 0x06,
        0x0E, 0x09, 0x15, 0x19, 0x03, 0x18, 0x00, 0x0B];

$self->{unk_bitfield}->{0x562968} = [
        0x12, 0x10, 0x11, 0x15, 0x03, 0x0A, 0x14, 0x05,
        0x1D, 0x07, 0x17, 0x0D, 0x09, 0x08, 0x1B, 0x1F,
        0x0B, 0x06, 0x19, 0x0E, 0x18, 0x04, 0x00, 0x02,
        0x1E, 0x1C, 0x01, 0x0C, 0x1A, 0x0F, 0x13, 0x16];

$self->{unk_bitfield}->{0x55F214} = [
        0x19, 0x00, 0x10, 0x18, 0x09, 0x11, 0x13, 0x1D,
        0x08, 0x1A, 0x02, 0x05, 0x03, 0x17, 0x12, 0x01,
        0x1F, 0x14, 0x06, 0x07, 0x15, 0x0D, 0x0F, 0x0B,
        0x0E, 0x16, 0x1E, 0x04, 0x1B, 0x0A, 0x0C, 0x1C];

$self->{unk_bitfield}->{0x55F380} = [
        0x18, 0x08, 0x14, 0x17, 0x03, 0x10, 0x19, 0x04,
        0x0D, 0x1C, 0x06, 0x1D, 0x1E, 0x12, 0x11, 0x0B,
        0x0F, 0x02, 0x0E, 0x1B, 0x13, 0x05, 0x07, 0x16,
        0x15, 0x0A, 0x0C, 0x1A, 0x00, 0x01, 0x1F, 0x09];

$self->{unk_bitfield}->{0x55F3F8} = [
        0x17, 0x1F, 0x0E, 0x05, 0x13, 0x0C, 0x14, 0x1A,
        0x0F, 0x01, 0x12, 0x1C, 0x00, 0x07, 0x0D, 0x02,
        0x10, 0x16, 0x04, 0x11, 0x1D, 0x03, 0x1E, 0x18,
        0x06, 0x15, 0x0A, 0x19, 0x09, 0x08, 0x1B, 0x0B];

$self->{unk_bitfield}->{0x562948} = [
        0x0F, 0x09, 0x1E, 0x11, 0x0D, 0x08, 0x10, 0x00,
        0x01, 0x1F, 0x1D, 0x1C, 0x12, 0x04, 0x07, 0x05,
        0x19, 0x14, 0x1B, 0x02, 0x1A, 0x15, 0x17, 0x16,
        0x18, 0x0B, 0x0A, 0x13, 0x0C, 0x0E, 0x03, 0x06];

$self->{unk_bitfield}->{0x561258} = [
        0x03, 0x11, 0x07, 0x1B, 0x0F, 0x14, 0x0C, 0x01,
        0x04, 0x02, 0x09, 0x0A, 0x05, 0x12, 0x06, 0x1F,
        0x1C, 0x0E, 0x0D, 0x15, 0x18, 0x08, 0x00, 0x10,
        0x1E, 0x1D, 0x17, 0x19, 0x13, 0x16, 0x0B, 0x1A];

$self->{unk_bitfield}->{0x561278} = [
        0x13, 0x1B, 0x06, 0x11, 0x1C, 0x07, 0x08, 0x0E,
        0x10, 0x05, 0x09, 0x18, 0x04, 0x15, 0x1E, 0x0F,
        0x1F, 0x12, 0x02, 0x00, 0x17, 0x19, 0x1A, 0x0D,
        0x03, 0x0C, 0x0A, 0x1D, 0x14, 0x01, 0x16, 0x0B];

$self->{unk_bitfield}->{0x561298} = [
        0x09, 0x0F, 0x00, 0x15, 0x12, 0x17, 0x1A, 0x0D,
        0x1C, 0x0B, 0x01, 0x0A, 0x05, 0x1E, 0x1D, 0x0C,
        0x1B, 0x08, 0x19, 0x18, 0x14, 0x07, 0x0E, 0x03,
        0x10, 0x16, 0x11, 0x1F, 0x04, 0x06, 0x02, 0x13];

$self->{unk_bitfield}->{0x5613A0} = [
        0x11, 0x10, 0x04, 0x1D, 0x08, 0x15, 0x1A, 0x1B,
        0x14, 0x18, 0x0F, 0x17, 0x16, 0x07, 0x1E, 0x0E,
        0x12, 0x0A, 0x13, 0x0B, 0x0C, 0x00, 0x06, 0x02,
        0x1F, 0x19, 0x09, 0x1C, 0x01, 0x0D, 0x03, 0x05];

$self->{unk_bitfield}->{0x5613C0} = [
        0x00, 0x05, 0x10, 0x1C, 0x0C, 0x1A, 0x04, 0x1B,
        0x0A, 0x0D, 0x14, 0x0B, 0x07, 0x03, 0x12, 0x1E,
        0x06, 0x11, 0x01, 0x08, 0x15, 0x09, 0x1F, 0x0F,
        0x19, 0x18, 0x16, 0x02, 0x13, 0x0E, 0x17, 0x1D];

$self->{unk_bitfield}->{0x5613E4} = [
        0x14, 0x0A, 0x1C, 0x00, 0x0C, 0x1F, 0x1E, 0x0B,
        0x12, 0x1D, 0x17, 0x08, 0x07, 0x04, 0x09, 0x10,
        0x03, 0x1B, 0x0E, 0x1A, 0x05, 0x0D, 0x11, 0x15,
        0x18, 0x02, 0x06, 0x01, 0x19, 0x16, 0x13, 0x0F];

$self->{unk_bitfield}->{0x561404} = [
        0x1E, 0x12, 0x06, 0x1D, 0x15, 0x1F, 0x13, 0x0B,
        0x10, 0x0D, 0x1C, 0x01, 0x0A, 0x0E, 0x02, 0x19,
        0x04, 0x1A, 0x03, 0x11, 0x00, 0x16, 0x0C, 0x17,
        0x14, 0x08, 0x18, 0x05, 0x09, 0x0F, 0x1B, 0x07];

$self->{unk_bitfield}->{0x5615A0} = [
        0x1A, 0x18, 0x12, 0x15, 0x00, 0x1C, 0x01, 0x0B,
        0x19, 0x1B, 0x1F, 0x11, 0x07, 0x10, 0x1E, 0x06,
        0x17, 0x04, 0x0A, 0x0E, 0x0D, 0x0C, 0x16, 0x08,
        0x02, 0x03, 0x13, 0x14, 0x09, 0x1D, 0x05, 0x0F];

$self->{unk_bitfield}->{0x561628} = [
        0x1C, 0x1D, 0x07, 0x12, 0x18, 0x1A, 0x19, 0x09,
        0x0F, 0x14, 0x1F, 0x0B, 0x13, 0x04, 0x0E, 0x1E,
        0x0C, 0x0D, 0x01, 0x17, 0x1B, 0x16, 0x0A, 0x05,
        0x15, 0x10, 0x11, 0x08, 0x00, 0x03, 0x06, 0x02];

$self->{unk_bitfield}->{0x561748} = [
        0x1A, 0x1C, 0x09, 0x17, 0x1B, 0x0B, 0x16, 0x1E,
        0x14, 0x0C, 0x12, 0x0E, 0x05, 0x03, 0x1F, 0x15,
        0x19, 0x0D, 0x10, 0x13, 0x0A, 0x01, 0x00, 0x11,
        0x02, 0x08, 0x0F, 0x18, 0x07, 0x04, 0x1D, 0x06];

$self->{unk_bitfield}->{0x561768} = [
        0x07, 0x03, 0x15, 0x0B, 0x02, 0x11, 0x17, 0x14,
        0x05, 0x10, 0x0A, 0x0F, 0x01, 0x1C, 0x1D, 0x0E,
        0x12, 0x06, 0x18, 0x16, 0x1A, 0x09, 0x13, 0x19,
        0x1B, 0x00, 0x08, 0x0D, 0x0C, 0x1E, 0x04, 0x1F];

$self->{unk_bitfield}->{0x5617F0} = [
        0x1D, 0x13, 0x1B, 0x10, 0x07, 0x03, 0x0A, 0x02,
        0x00, 0x0C, 0x0E, 0x0B, 0x0D, 0x18, 0x12, 0x1F,
        0x1A, 0x04, 0x15, 0x11, 0x1E, 0x08, 0x1C, 0x14,
        0x19, 0x05, 0x0F, 0x17, 0x06, 0x01, 0x09, 0x16];

$self->{unk_bitfield}->{0x561EA0} = [
        0x09, 0x0F, 0x10, 0x0C, 0x03, 0x15, 0x07, 0x17,
        0x0E, 0x0B, 0x1D, 0x08, 0x19, 0x11, 0x00, 0x0A,
        0x01, 0x06, 0x18, 0x16, 0x0D, 0x13, 0x14, 0x12,
        0x02, 0x1B, 0x1A, 0x04, 0x05, 0x1F, 0x1C, 0x1E];

$self->{unk_bitfield}->{0x561F38} = [
        0x0F, 0x09, 0x02, 0x06, 0x18, 0x0B, 0x1E, 0x05,
        0x11, 0x1D, 0x16, 0x01, 0x13, 0x10, 0x0E, 0x1A,
        0x1B, 0x00, 0x0D, 0x08, 0x15, 0x14, 0x19, 0x17,
        0x03, 0x1F, 0x0A, 0x12, 0x0C, 0x07, 0x04, 0x1C];

$self->{unk_bitfield}->{0x561F58} = [
        0x0D, 0x1C, 0x1F, 0x15, 0x0F, 0x14, 0x1B, 0x12,
        0x09, 0x0B, 0x19, 0x07, 0x11, 0x16, 0x0C, 0x04,
        0x13, 0x05, 0x1D, 0x03, 0x0E, 0x0A, 0x08, 0x1E,
        0x01, 0x06, 0x18, 0x17, 0x10, 0x1A, 0x02, 0x00];

$self->{unk_bitfield}->{0x561F7C} = [
        0x05, 0x15, 0x1D, 0x02, 0x0F, 0x03, 0x17, 0x1A,
        0x0A, 0x00, 0x1F, 0x12, 0x0E, 0x11, 0x1B, 0x13,
        0x0B, 0x0D, 0x09, 0x18, 0x1E, 0x08, 0x14, 0x07,
        0x0C, 0x04, 0x16, 0x19, 0x1C, 0x06, 0x10, 0x01];

$self->{unk_bitfield}->{0x561F9C} = [
        0x06, 0x1E, 0x0C, 0x11, 0x13, 0x08, 0x15, 0x01,
        0x1D, 0x03, 0x0F, 0x19, 0x18, 0x04, 0x00, 0x14,
        0x12, 0x1A, 0x0B, 0x0E, 0x02, 0x1B, 0x07, 0x05,
        0x1F, 0x17, 0x09, 0x0A, 0x0D, 0x16, 0x10, 0x1C];

$self->{unk_bitfield}->{0x562198} = [
        0x0E, 0x16, 0x18, 0x11, 0x0C, 0x01, 0x12, 0x1F,
        0x08, 0x15, 0x0A, 0x06, 0x1C, 0x1E, 0x02, 0x1A,
        0x17, 0x03, 0x07, 0x13, 0x05, 0x19, 0x10, 0x0F,
        0x0D, 0x14, 0x09, 0x0B, 0x1B, 0x00, 0x1D, 0x04];

$self->{unk_bitfield}->{0x5622F0} = [
        0x00, 0x1B, 0x17, 0x19, 0x1D, 0x11, 0x0D, 0x1A,
        0x13, 0x03, 0x1E, 0x09, 0x10, 0x0E, 0x15, 0x05,
        0x0B, 0x1C, 0x1F, 0x08, 0x0A, 0x06, 0x01, 0x0F,
        0x16, 0x14, 0x02, 0x04, 0x07, 0x18, 0x12, 0x0C];

$self->{unk_bitfield}->{0x562864} = [
        0x17, 0x07, 0x0D, 0x16, 0x00, 0x1B, 0x1F, 0x09,
        0x10, 0x11, 0x14, 0x0A, 0x02, 0x06, 0x13, 0x0C,
        0x08, 0x1E, 0x0F, 0x12, 0x05, 0x15, 0x19, 0x01,
        0x1C, 0x1A, 0x03, 0x18, 0x04, 0x0B, 0x1D, 0x0E];

$self->{unk_bitfield}->{0x56238C} = [
        0x1B, 0x0B, 0x0C, 0x06, 0x1F, 0x17, 0x04, 0x1A,
        0x1E, 0x02, 0x0F, 0x16, 0x0E, 0x09, 0x10, 0x01,
        0x13, 0x19, 0x11, 0x00, 0x0A, 0x05, 0x03, 0x1C,
        0x18, 0x1D, 0x14, 0x0D, 0x07, 0x08, 0x15, 0x12];

$self->{unk_bitfield}->{0x5623AC} = [
        0x02, 0x1A, 0x17, 0x1D, 0x01, 0x03, 0x13, 0x1E,
        0x05, 0x18, 0x06, 0x0A, 0x0C, 0x04, 0x1B, 0x00,
        0x1C, 0x09, 0x1F, 0x16, 0x07, 0x0F, 0x0B, 0x0E,
        0x14, 0x12, 0x0D, 0x10, 0x19, 0x11, 0x08, 0x15];

$self->{unk_bitfield}->{0x5623CC} = [
        0x0E, 0x14, 0x12, 0x1E, 0x1C, 0x02, 0x06, 0x16,
        0x18, 0x0D, 0x17, 0x0C, 0x1D, 0x11, 0x08, 0x19,
        0x07, 0x0F, 0x13, 0x04, 0x03, 0x1B, 0x0B, 0x1F,
        0x1A, 0x0A, 0x05, 0x10, 0x00, 0x01, 0x15, 0x09];

$self->{unk_bitfield}->{0x5623EC} = [
        0x00, 0x0D, 0x03, 0x02, 0x11, 0x04, 0x18, 0x0B,
        0x14, 0x1D, 0x1C, 0x13, 0x1B, 0x17, 0x10, 0x15,
        0x01, 0x19, 0x07, 0x09, 0x1A, 0x16, 0x12, 0x1E,
        0x08, 0x06, 0x0C, 0x0E, 0x1F, 0x0F, 0x0A, 0x05];

$self->{unk_bitfield}->{0x562908} = [
        0x1B, 0x14, 0x12, 0x15, 0x11, 0x1D, 0x17, 0x19,
        0x10, 0x09, 0x08, 0x06, 0x1A, 0x16, 0x07, 0x13,
        0x1F, 0x0B, 0x1C, 0x05, 0x0E, 0x00, 0x18, 0x0A,
        0x04, 0x01, 0x03, 0x0C, 0x0D, 0x1E, 0x02, 0x0F];

$self->{unk_bitfield}->{0x56249C} = [
        0x15, 0x00, 0x10, 0x0B, 0x1D, 0x0A, 0x06, 0x1C,
        0x0D, 0x1F, 0x17, 0x0F, 0x03, 0x14, 0x13, 0x12,
        0x1B, 0x18, 0x08, 0x1E, 0x16, 0x09, 0x1A, 0x04,
        0x02, 0x0C, 0x0E, 0x01, 0x07, 0x19, 0x11, 0x05];

$self->{unk_bitfield}->{0x562544} = [
        0x07, 0x06, 0x11, 0x08, 0x0C, 0x1F, 0x19, 0x02,
        0x14, 0x04, 0x0D, 0x18, 0x1A, 0x05, 0x17, 0x13,
        0x1C, 0x1B, 0x15, 0x03, 0x01, 0x0F, 0x16, 0x1E,
        0x1D, 0x10, 0x00, 0x12, 0x0B, 0x0E, 0x09, 0x0A];

$self->{unk_bitfield}->{0x562564} = [
        0x11, 0x01, 0x1F, 0x06, 0x1A, 0x04, 0x02, 0x09,
        0x05, 0x0D, 0x0B, 0x18, 0x0E, 0x12, 0x1B, 0x17,
        0x07, 0x08, 0x1D, 0x1E, 0x14, 0x19, 0x16, 0x15,
        0x03, 0x0C, 0x00, 0x10, 0x0A, 0x1C, 0x0F, 0x13];

$self->{unk_bitfield}->{0x5625A0} = [
        0x1F, 0x0B, 0x13, 0x00, 0x16, 0x15, 0x14, 0x0A,
        0x1D, 0x05, 0x1E, 0x1A, 0x0F, 0x04, 0x0E, 0x01,
        0x19, 0x07, 0x02, 0x12, 0x0C, 0x17, 0x08, 0x09,
        0x03, 0x11, 0x18, 0x10, 0x1C, 0x1B, 0x06, 0x0D];

$self->{unk_bitfield}->{0x5625C0} = [
        0x02, 0x1B, 0x0C, 0x17, 0x1F, 0x05, 0x15, 0x1E,
        0x16, 0x09, 0x1A, 0x12, 0x0F, 0x1C, 0x18, 0x0A,
        0x19, 0x10, 0x0D, 0x13, 0x04, 0x11, 0x08, 0x14,
        0x1D, 0x0E, 0x06, 0x00, 0x01, 0x07, 0x0B, 0x03];

$self->{unk_bitfield}->{0x5625E0} = [
        0x00, 0x06, 0x0B, 0x08, 0x0C, 0x04, 0x1A, 0x1C,
        0x05, 0x1E, 0x14, 0x03, 0x0A, 0x18, 0x12, 0x1D,
        0x16, 0x1F, 0x07, 0x09, 0x0F, 0x0E, 0x17, 0x13,
        0x11, 0x19, 0x10, 0x0D, 0x1B, 0x02, 0x01, 0x15];

$self->{unk_bitfield}->{0x562600} = [
        0x12, 0x03, 0x1A, 0x15, 0x04, 0x19, 0x0B, 0x1B,
        0x17, 0x1E, 0x0D, 0x05, 0x11, 0x14, 0x1C, 0x00,
        0x18, 0x10, 0x0A, 0x06, 0x0E, 0x08, 0x02, 0x07,
        0x13, 0x09, 0x16, 0x1D, 0x0F, 0x0C, 0x01, 0x1F];

$self->{unk_bitfield}->{0x562620} = [
        0x1E, 0x1B, 0x11, 0x07, 0x08, 0x06, 0x18, 0x17,
        0x0D, 0x0F, 0x12, 0x03, 0x1D, 0x04, 0x0A, 0x1A,
        0x0C, 0x13, 0x14, 0x1F, 0x0B, 0x19, 0x10, 0x01,
        0x16, 0x05, 0x1C, 0x0E, 0x02, 0x00, 0x09, 0x15];

$self->{unk_bitfield}->{0x562640} = [
        0x0F, 0x1D, 0x17, 0x16, 0x0D, 0x05, 0x13, 0x1F,
        0x1B, 0x09, 0x1C, 0x1E, 0x15, 0x01, 0x06, 0x08,
        0x0C, 0x10, 0x0B, 0x02, 0x04, 0x0A, 0x07, 0x1A,
        0x18, 0x0E, 0x03, 0x11, 0x12, 0x14, 0x19, 0x00];

$self->{unk_bitfield}->{0x562660} = [
        0x0F, 0x04, 0x1D, 0x1B, 0x15, 0x10, 0x01, 0x0B,
        0x00, 0x17, 0x13, 0x07, 0x1E, 0x1F, 0x08, 0x0A,
        0x19, 0x09, 0x05, 0x06, 0x0C, 0x1A, 0x14, 0x16,
        0x0E, 0x18, 0x03, 0x1C, 0x12, 0x11, 0x0D, 0x02];


# fn to unk table lookup mapping
$self->{fn_unk_lookup}->{0x4016F6} = 0x538920;
$self->{fn_unk_lookup}->{0x401AF7} = 0x538A20;
$self->{fn_unk_lookup}->{0x4095B6} = 0x539320;
$self->{fn_unk_lookup}->{0x40F90B} = 0x539868;
$self->{fn_unk_lookup}->{0x40B383} = 0x5395A8;
$self->{fn_unk_lookup}->{0x4121AE} = 0x539BA8;
$self->{fn_unk_lookup}->{0x4F178C} = 0x547818;
$self->{fn_unk_lookup}->{0x4F268A} = 0x547978;
$self->{fn_unk_lookup}->{0x413C0D} = 0x539D48;
$self->{fn_unk_lookup}->{0x4147E7} = 0x539F60;
$self->{fn_unk_lookup}->{0x41489C} = 0x53A060;
$self->{fn_unk_lookup}->{0x417E14} = 0x53A588;
$self->{fn_unk_lookup}->{0x417EC9} = 0x53A688;
$self->{fn_unk_lookup}->{0x419FF2} = 0x53A7C8;
$self->{fn_unk_lookup}->{0x41A7A4} = 0x53A8C8;
$self->{fn_unk_lookup}->{0x41AA13} = 0x53AA20;
$self->{fn_unk_lookup}->{0x4F5718} = 0x547B00;
$self->{fn_unk_lookup}->{0x4ED672} = 0x547548;
$self->{fn_unk_lookup}->{0x4227C0} = 0x53B1D0;
$self->{fn_unk_lookup}->{0x422875} = 0x53B2D0;
$self->{fn_unk_lookup}->{0x422DAD} = 0x53B418;
$self->{fn_unk_lookup}->{0x423782} = 0x53B550;
$self->{fn_unk_lookup}->{0x425258} = 0x53B718;
$self->{fn_unk_lookup}->{0x425603} = 0x53B838;
$self->{fn_unk_lookup}->{0x426D40} = 0x53BC10;
$self->{fn_unk_lookup}->{0x426DF5} = 0x53BD10;
$self->{fn_unk_lookup}->{0x42A19A} = 0x53BE50;
$self->{fn_unk_lookup}->{0x42AAF4} = 0x53BF68;
$self->{fn_unk_lookup}->{0x42B45C} = 0x53C0E8;
$self->{fn_unk_lookup}->{0x42E63C} = 0x53C240;
$self->{fn_unk_lookup}->{0x42EE57} = 0x53C368;
$self->{fn_unk_lookup}->{0x436D5F} = 0x53C7B0;
$self->{fn_unk_lookup}->{0x437312} = 0x53C8B8;
$self->{fn_unk_lookup}->{0x437863} = 0x53C9E8;
$self->{fn_unk_lookup}->{0x437918} = 0x53CAE8;
$self->{fn_unk_lookup}->{0x43974A} = 0x53CD28;
$self->{fn_unk_lookup}->{0x4EA6BD} = 0x547230;
$self->{fn_unk_lookup}->{0x439D1C} = 0x53CEB0;
$self->{fn_unk_lookup}->{0x43E32E} = 0x53D168;
$self->{fn_unk_lookup}->{0x43FD05} = 0x53D2D0;
$self->{fn_unk_lookup}->{0x43FDBA} = 0x53D3D0;
$self->{fn_unk_lookup}->{0x4424A6} = 0x53D798;
$self->{fn_unk_lookup}->{0x443762} = 0x53D970;
$self->{fn_unk_lookup}->{0x443817} = 0x53DA70;
$self->{fn_unk_lookup}->{0x443FB8} = 0x53DBD0;
$self->{fn_unk_lookup}->{0x445471} = 0x53DD70;
$self->{fn_unk_lookup}->{0x44C4F8} = 0x53E0D0;
$self->{fn_unk_lookup}->{0x453D17} = 0x540278;
$self->{fn_unk_lookup}->{0x45516E} = 0x5406E8;
$self->{fn_unk_lookup}->{0x459542} = 0x5409E8;
$self->{fn_unk_lookup}->{0x45F3BE} = 0x540D18;
$self->{fn_unk_lookup}->{0x46FCC5} = 0x541218;
$self->{fn_unk_lookup}->{0x4F764C} = 0x547D98;
$self->{fn_unk_lookup}->{0x4F7AF6} = 0x547F10;
$self->{fn_unk_lookup}->{0x4011A5} = 0x5387B0;
$self->{fn_unk_lookup}->{0x40966B} = 0x539420;
$self->{fn_unk_lookup}->{0x4F6652} = 0x547C10;
$self->{fn_unk_lookup}->{0x4726D7} = 0x541400;
$self->{fn_unk_lookup}->{0x475502} = 0x541588;
$self->{fn_unk_lookup}->{0x4755C1} = 0x541688;
$self->{fn_unk_lookup}->{0x47A492} = 0x541988;
$self->{fn_unk_lookup}->{0x47C5E5} = 0x541B30;
$self->{fn_unk_lookup}->{0x47C6A4} = 0x541C30;
$self->{fn_unk_lookup}->{0x47E7EB} = 0x541DB8;
$self->{fn_unk_lookup}->{0x481DCE} = 0x541FE0;
$self->{fn_unk_lookup}->{0x481EBC} = 0x5420E0;
$self->{fn_unk_lookup}->{0x483131} = 0x542220;
$self->{fn_unk_lookup}->{0x41AB56} = 0x53AB20;
$self->{fn_unk_lookup}->{0x48A32D} = 0x542738;
$self->{fn_unk_lookup}->{0x492896} = 0x542DF8;
$self->{fn_unk_lookup}->{0x4978E5} = 0x5430B8;
$self->{fn_unk_lookup}->{0x49898F} = 0x543220;
$self->{fn_unk_lookup}->{0x49B296} = 0x543580;
$self->{fn_unk_lookup}->{0x49B355} = 0x543680;
$self->{fn_unk_lookup}->{0x49F67A} = 0x543848;
$self->{fn_unk_lookup}->{0x4A0507} = 0x543990;
$self->{fn_unk_lookup}->{0x4A7E6C} = 0x543BF8;
$self->{fn_unk_lookup}->{0x4A8411} = 0x543D08;
$self->{fn_unk_lookup}->{0x4B38C8} = 0x544130;
$self->{fn_unk_lookup}->{0x4B44E0} = 0x544298;
$self->{fn_unk_lookup}->{0x4E6859} = 0x546D50;
$self->{fn_unk_lookup}->{0x4B84A6} = 0x544548;
$self->{fn_unk_lookup}->{0x4B955C} = 0x544700;
$self->{fn_unk_lookup}->{0x4BB695} = 0x5448A8;
$self->{fn_unk_lookup}->{0x4BB97C} = 0x544A28;
$self->{fn_unk_lookup}->{0x4EEA50} = 0x547670;
$self->{fn_unk_lookup}->{0x4FB7E6} = 0x5480C8;
$self->{fn_unk_lookup}->{0x4E5A93} = 0x546AF0;
$self->{fn_unk_lookup}->{0x4E5B52} = 0x546BF0;
$self->{fn_unk_lookup}->{0x4BFB7A} = 0x544D20;
$self->{fn_unk_lookup}->{0x4BFC39} = 0x544E20;
$self->{fn_unk_lookup}->{0x4E99FC} = 0x546FD8;
$self->{fn_unk_lookup}->{0x4E9ABB} = 0x5470D8;
$self->{fn_unk_lookup}->{0x4E3B1E} = 0x546970;
$self->{fn_unk_lookup}->{0x4DFE2A} = 0x5467D8;
$self->{fn_unk_lookup}->{0x4DDE19} = 0x546690;
$self->{fn_unk_lookup}->{0x4DCF8C} = 0x546548;
$self->{fn_unk_lookup}->{0x4D247C} = 0x545C58;
$self->{fn_unk_lookup}->{0x4CB803} = 0x545AA0;
$self->{fn_unk_lookup}->{0x4D3679} = 0x545D78;
$self->{fn_unk_lookup}->{0x4D3738} = 0x545E78;
$self->{fn_unk_lookup}->{0x4D4169} = 0x545FF8;
$self->{fn_unk_lookup}->{0x4D5E3D} = 0x546190;
$self->{fn_unk_lookup}->{0x4D7968} = 0x546290;
$self->{fn_unk_lookup}->{0x4D804F} = 0x546398;

# lookup
$self->{unk_lookup}->{0x538920} = [
        ];

$self->{unk_lookup}->{0x538A20} = [
        ];

$self->{unk_lookup}->{0x539320} = [
        ];

$self->{unk_lookup}->{0x539868} = [
        ];

$self->{unk_lookup}->{0x5395A8} = [
        ];

$self->{unk_lookup}->{0x539BA8} = [
        ];

$self->{unk_lookup}->{0x547818} = [
        ];

$self->{unk_lookup}->{0x547978} = [
        ];

$self->{unk_lookup}->{0x539D48} = [
        ];

$self->{unk_lookup}->{0x539F60} = [
        ];

$self->{unk_lookup}->{0x53A060} = [
        ];

$self->{unk_lookup}->{0x53A588} = [
        ];

$self->{unk_lookup}->{0x53A688} = [
        ];

$self->{unk_lookup}->{0x53A7C8} = [
        ];

$self->{unk_lookup}->{0x53A8C8} = [
        ];

$self->{unk_lookup}->{0x53AA20} = [
        ];

$self->{unk_lookup}->{0x547B00} = [
        ];

$self->{unk_lookup}->{0x547548} = [
        ];

$self->{unk_lookup}->{0x53B1D0} = [
        ];

$self->{unk_lookup}->{0x53B2D0} = [
        ];

$self->{unk_lookup}->{0x53B418} = [
        ];

$self->{unk_lookup}->{0x53B550} = [
        ];

$self->{unk_lookup}->{0x53B718} = [
        ];

$self->{unk_lookup}->{0x53B838} = [
        ];

$self->{unk_lookup}->{0x53BC10} = [
        ];

$self->{unk_lookup}->{0x53BD10} = [
        ];

$self->{unk_lookup}->{0x53BE50} = [
        ];

$self->{unk_lookup}->{0x53BF68} = [
        ];

$self->{unk_lookup}->{0x53C0E8} = [
        ];

$self->{unk_lookup}->{0x53C240} = [
        ];

$self->{unk_lookup}->{0x53C368} = [
        ];

$self->{unk_lookup}->{0x53C7B0} = [
        ];

$self->{unk_lookup}->{0x53C8B8} = [
        ];

$self->{unk_lookup}->{0x53C9E8} = [
        ];

$self->{unk_lookup}->{0x53CAE8} = [
        ];

$self->{unk_lookup}->{0x53CD28} = [
        ];

$self->{unk_lookup}->{0x547230} = [
        ];

$self->{unk_lookup}->{0x53CEB0} = [
        ];

$self->{unk_lookup}->{0x53D168} = [
        ];

$self->{unk_lookup}->{0x53D2D0} = [
        ];

$self->{unk_lookup}->{0x53D3D0} = [
        ];

$self->{unk_lookup}->{0x53D798} = [
        ];

$self->{unk_lookup}->{0x53D970} = [
        ];

$self->{unk_lookup}->{0x53DA70} = [
        ];

$self->{unk_lookup}->{0x53DBD0} = [
        ];

$self->{unk_lookup}->{0x53DD70} = [
        ];

$self->{unk_lookup}->{0x53E0D0} = [
        ];

$self->{unk_lookup}->{0x540278} = [
        ];

$self->{unk_lookup}->{0x5406E8} = [
        ];

$self->{unk_lookup}->{0x5409E8} = [
        ];

$self->{unk_lookup}->{0x540D18} = [
        ];

$self->{unk_lookup}->{0x541218} = [
        ];

$self->{unk_lookup}->{0x547D98} = [
        ];

$self->{unk_lookup}->{0x547F10} = [
        ];

$self->{unk_lookup}->{0x5387B0} = [
        ];

$self->{unk_lookup}->{0x539420} = [
        ];

$self->{unk_lookup}->{0x547C10} = [
        ];

$self->{unk_lookup}->{0x541400} = [
        ];

$self->{unk_lookup}->{0x541588} = [
        ];

$self->{unk_lookup}->{0x541688} = [
        ];

$self->{unk_lookup}->{0x541988} = [
        ];

$self->{unk_lookup}->{0x541B30} = [
        ];

$self->{unk_lookup}->{0x541C30} = [
        ];

$self->{unk_lookup}->{0x541DB8} = [
        ];

$self->{unk_lookup}->{0x541FE0} = [
        ];

$self->{unk_lookup}->{0x5420E0} = [
        ];

$self->{unk_lookup}->{0x542220} = [
        ];

$self->{unk_lookup}->{0x53AB20} = [
        ];

$self->{unk_lookup}->{0x542738} = [
        ];

$self->{unk_lookup}->{0x542DF8} = [
        ];

$self->{unk_lookup}->{0x5430B8} = [
        ];

$self->{unk_lookup}->{0x543220} = [
        ];

$self->{unk_lookup}->{0x543580} = [
        ];

$self->{unk_lookup}->{0x543680} = [
        ];

$self->{unk_lookup}->{0x543848} = [
        ];

$self->{unk_lookup}->{0x543990} = [
        ];

$self->{unk_lookup}->{0x543BF8} = [
        ];

$self->{unk_lookup}->{0x543D08} = [
        ];

$self->{unk_lookup}->{0x544130} = [
        ];

$self->{unk_lookup}->{0x544298} = [
        ];

$self->{unk_lookup}->{0x546D50} = [
        ];

$self->{unk_lookup}->{0x544548} = [
        ];

$self->{unk_lookup}->{0x544700} = [
        ];

$self->{unk_lookup}->{0x5448A8} = [
        ];

$self->{unk_lookup}->{0x544A28} = [
        ];

$self->{unk_lookup}->{0x547670} = [
        ];

$self->{unk_lookup}->{0x5480C8} = [
        ];

$self->{unk_lookup}->{0x546AF0} = [
        ];

$self->{unk_lookup}->{0x546BF0} = [
        ];

$self->{unk_lookup}->{0x544D20} = [
        ];

$self->{unk_lookup}->{0x544E20} = [
        ];

$self->{unk_lookup}->{0x546FD8} = [
        ];

$self->{unk_lookup}->{0x5470D8} = [
        ];

$self->{unk_lookup}->{0x546970} = [
        ];

$self->{unk_lookup}->{0x5467D8} = [
        ];

$self->{unk_lookup}->{0x546690} = [
        ];

$self->{unk_lookup}->{0x546548} = [
        ];

$self->{unk_lookup}->{0x545C58} = [
        ];

$self->{unk_lookup}->{0x545AA0} = [
        ];

$self->{unk_lookup}->{0x545D78} = [
        ];

$self->{unk_lookup}->{0x545E78} = [
        ];

$self->{unk_lookup}->{0x545FF8} = [
        ];

$self->{unk_lookup}->{0x546190} = [
        ];

$self->{unk_lookup}->{0x546290} = [
        ];

$self->{unk_lookup}->{0x546398} = [
        ];

# muladd
$self->{unk_muladd}->{0x401117} = [0x36056CD7, 0x4387];
$self->{unk_muladd}->{0x40A006} = [0x4ABB534D, 0x3769];
$self->{unk_muladd}->{0x40CE91} = [0x3C23132D, 0x339B];
$self->{unk_muladd}->{0x410C58} = [0x7C466A4B, 0x29BF];
$self->{unk_muladd}->{0x414951} = [0x7E823B21, 0x6BB3];
$self->{unk_muladd}->{0x4F07A0} = [0x5B756AB9, 0x7E9B];
$self->{unk_muladd}->{0x416ED1} = [0x2417591B, 0x177B];
$self->{unk_muladd}->{0x416F5F} = [0x57F27C5F, 0x2433];
$self->{unk_muladd}->{0x419F64} = [0x58E937F9, 0x1075];
$self->{unk_muladd}->{0x41AAC8} = [0x0B4C3D13, 0x1597];
$self->{unk_muladd}->{0x4F422E} = [0x689B4017, 0x3CFB];
$self->{unk_muladd}->{0x41CAFA} = [0x05B611AB, 0x570B];
$self->{unk_muladd}->{0x41CB88} = [0x0DA5334F, 0x3AC7];
$self->{unk_muladd}->{0x421FAD} = [0x314C2CD1, 0x2B5B];
$self->{unk_muladd}->{0x4FC58A} = [0x28EA041F, 0x638F];
$self->{unk_muladd}->{0x422E62} = [0x511537CB, 0x7135];
$self->{unk_muladd}->{0x423203} = [0x1CF71007, 0x5E17];
$self->{unk_muladd}->{0x4251CA} = [0x4D595519, 0x1A7D];
$self->{unk_muladd}->{0x429822} = [0x1EDC0BA3, 0x7531];
$self->{unk_muladd}->{0x42ADFB} = [0x2F076EEB, 0x5BCF];
$self->{unk_muladd}->{0x42B511} = [0x41CD726F, 0x3F79];
$self->{unk_muladd}->{0x42C5A6} = [0x19B32B03, 0x4AD1];
$self->{unk_muladd}->{0x42D7BF} = [0x39731111, 0x419B];
$self->{unk_muladd}->{0x42EF0C} = [0x3CC0256B, 0x7CE7];
$self->{unk_muladd}->{0x43321B} = [0x228F7FB5, 0x472D];
$self->{unk_muladd}->{0x434B9B} = [0x32DA290B, 0x7745];
$self->{unk_muladd}->{0x436CD1} = [0x5C814F8B, 0x227F];
$self->{unk_muladd}->{0x436E14} = [0x0B496F6D, 0x412D];
$self->{unk_muladd}->{0x44305D} = [0x102C215B, 0x0581];
$self->{unk_muladd}->{0x44406D} = [0x19511111, 0x12C1];
$self->{unk_muladd}->{0x445526} = [0x2A6E2953, 0x6977];
$self->{unk_muladd}->{0x44F36E} = [0x646C21EB, 0x43E5];
$self->{unk_muladd}->{0x4538AE} = [0x5E6105DB, 0x1605];
$self->{unk_muladd}->{0x4ED483} = [0x48AF73CB, 0x0A67];
$self->{unk_muladd}->{0x45A2FA} = [0x5FC41A57, 0x0693];
$self->{unk_muladd}->{0x45A732} = [0x17154387, 0x2489];
$self->{unk_muladd}->{0x471022} = [0x00E813A5, 0x2CE5];
$self->{unk_muladd}->{0x4F6E70} = [0x3D707E25, 0x3827];
$self->{unk_muladd}->{0x4F6EFE} = [0x77A53E07, 0x6A5F];
$self->{unk_muladd}->{0x40E1CC} = [0x32CA58E3, 0x04F9];
$self->{unk_muladd}->{0x416416} = [0x218B2569, 0x5DB1];
$self->{unk_muladd}->{0x417004} = [0x7D1428CB, 0x3D];
$self->{unk_muladd}->{0x4722E7} = [0x1E3349F7, 0x41F5];
$self->{unk_muladd}->{0x47BE3C} = [0x5C221625, 0x576F];
$self->{unk_muladd}->{0x47CE8F} = [0x680B1F17, 0x12CD];
$self->{unk_muladd}->{0x47E29E} = [0x12564D55, 0x32B9];
$self->{unk_muladd}->{0x47E753} = [0x21A67897, 0x6BAB];
$self->{unk_muladd}->{0x47ECD0} = [0x06405119, 0x7143];
$self->{unk_muladd}->{0x480895} = [0x46356F6B, 0x0A49];
$self->{unk_muladd}->{0x48092D} = [0x32C77969, 0x72F3];
$self->{unk_muladd}->{0x484929} = [0x74D52C55, 0x5F43];
$self->{unk_muladd}->{0x48A295} = [0x578F1047, 0x640B];
$self->{unk_muladd}->{0x49476D} = [0x4BFB6EF3, 0x704F];
$self->{unk_muladd}->{0x494805} = [0x1B4C7FE7, 0x5637];
$self->{unk_muladd}->{0x494C3E} = [0x04091A3B, 0x4917];
$self->{unk_muladd}->{0x49A264} = [0x127549D5, 0x579B];
$self->{unk_muladd}->{0x49A2FC} = [0x0AB54121, 0x7A47];
$self->{unk_muladd}->{0x49B5C2} = [0x6B080851, 0x7E8B];
$self->{unk_muladd}->{0x49F761} = [0x58A67753, 0x272B];
$self->{unk_muladd}->{0x49F7F9} = [0x1AB54AD7, 0x4D33];
$self->{unk_muladd}->{0x49FB5A} = [0x07D30A45, 0x0569];
$self->{unk_muladd}->{0x49FBF2} = [0x737616BF, 0x70C7];
$self->{unk_muladd}->{0x4A05C6} = [0x45C4485D, 0x2063];
$self->{unk_muladd}->{0x4A4D99} = [0x223A4FE3, 0x49A7];
$self->{unk_muladd}->{0x4A7F2B} = [0x4BC13C4F, 0x45C1];
$self->{unk_muladd}->{0x4A88EB} = [0x71406AB3, 0x7A5F];
$self->{unk_muladd}->{0x4A9638} = [0x009366D1, 0x22D1];
$self->{unk_muladd}->{0x4A96D0} = [0x587D1B75, 0x2CA5];
$self->{unk_muladd}->{0x4A9FDD} = [0x213A4BE7, 0x4499];
$self->{unk_muladd}->{0x4AA075} = [0x62653E89, 0x2D5D];
$self->{unk_muladd}->{0x4AF240} = [0x4F5F3257, 0x444F];
$self->{unk_muladd}->{0x4B1E08} = [0x4C0E2B2B, 0x19D3];
$self->{unk_muladd}->{0x4B1EA0} = [0x3F867B35, 0x7B3B];
$self->{unk_muladd}->{0x4B1FBD} = [0x32D25CB1, 0x3D6D];
$self->{unk_muladd}->{0x4B3830} = [0x50FA1C51, 0x5F4F];
$self->{unk_muladd}->{0x4B3FB5} = [0x14067C29, 0x10C5];
$self->{unk_muladd}->{0x4B459F} = [0x4A5558C5, 0x271F];
$self->{unk_muladd}->{0x4E6918} = [0x18837C9D, 0x6335];
$self->{unk_muladd}->{0x4B8565} = [0x03B87321, 0x7225];
$self->{unk_muladd}->{0x4BA417} = [0x2B36757D, 0x028D];
$self->{unk_muladd}->{0x4BBE02} = [0x3782017D, 0x05BF];
$self->{unk_muladd}->{0x4C0861} = [0x5EE51C43, 0x4553];
$self->{unk_muladd}->{0x4E3BDD} = [0x42B14C6F, 0x5531];
$self->{unk_muladd}->{0x4E7BC8} = [0x5C071D85, 0x2437];
$self->{unk_muladd}->{0x4DFEE9} = [0x29195861, 0x108B];
$self->{unk_muladd}->{0x418AE4} = [0x3FD30CF5, 0x7027];
$self->{unk_muladd}->{0x4C2035} = [0x457776B7, 0x24B3];
$self->{unk_muladd}->{0x4DCEF4} = [0x302F5B13, 0x371D];
$self->{unk_muladd}->{0x4C36CD} = [0x58692D47, 0x0671];
$self->{unk_muladd}->{0x4C4949} = [0x0F195B9B, 0x1369];
$self->{unk_muladd}->{0x4CBFEA} = [0x5BCF3275, 0x6E1B];
$self->{unk_muladd}->{0x4CC4E3} = [0x62725C5B, 0x16B9];
$self->{unk_muladd}->{0x4CC57B} = [0x5B950FDF, 0x2D35];
$self->{unk_muladd}->{0x4CEE8C} = [0x73BA5335, 0x1C13];
$self->{unk_muladd}->{0x4D23E4} = [0x4EED7B27, 0x38AB];
$self->{unk_muladd}->{0x4D2A11} = [0x627C7E0F, 0x7F01];
$self->{unk_muladd}->{0x4D2AA9} = [0x5D7E1F73, 0x2C0F];
$self->{unk_muladd}->{0x4CB8C2} = [0x55C9525F, 0x4659];
$self->{unk_muladd}->{0x4D3148} = [0x5DF66DDF, 0x7C25];
$self->{unk_muladd}->{0x4D7FB7} = [0x4392327B, 0x7E0D];
$self->{unk_muladd}->{0x4DAA64} = [0x3D8B0CB5, 0x640D];
$self->{unk_muladd}->{0x4DAAFC} = [0x32865601, 0x4D43];

# xor
$self->{unk_xor}->{0x40CE09} = 0x1D242DA5;
$self->{unk_xor}->{0x40E0BC} = 0x0191265C;
$self->{unk_xor}->{0x40E144} = 0x3DB979DB;
$self->{unk_xor}->{0x40F9C0} = 0x1A550E1E;
$self->{unk_xor}->{0x410BD0} = 0x2F140A2D;
$self->{unk_xor}->{0x413D64} = 0x2D3F30D3;
$self->{unk_xor}->{0x413CC2} = 0x1D1C4911;
$self->{unk_xor}->{0x416306} = 0x46BD7771;
$self->{unk_xor}->{0x41638E} = 0x51AE2B42;
$self->{unk_xor}->{0x4183F1} = 0x71422261;
$self->{unk_xor}->{0x41BA49} = 0x0FE07D38;
$self->{unk_xor}->{0x4F57CD} = 0x35413DF3;
$self->{unk_xor}->{0x41E838} = 0x47706008;
$self->{unk_xor}->{0x421F25} = 0x57611B36;
$self->{unk_xor}->{0x4FC502} = 0x1EF33946;
$self->{unk_xor}->{0x423291} = 0x583D4BCF;
$self->{unk_xor}->{0x423837} = 0x373E6856;
$self->{unk_xor}->{0x4256B8} = 0x0E2A36A7;
$self->{unk_xor}->{0x428877} = 0x53F3604F;
$self->{unk_xor}->{0x42A24F} = 0x10DF1038;
$self->{unk_xor}->{0x42AD73} = 0x4EDE0CAC;
$self->{unk_xor}->{0x42B2DC} = 0x6D86030F;
$self->{unk_xor}->{0x42B364} = 0x3F331713;
$self->{unk_xor}->{0x42BE26} = 0x0ECE0054;
$self->{unk_xor}->{0x42D869} = 0x54F7757A;
$self->{unk_xor}->{0x433193} = 0x79991847;
$self->{unk_xor}->{0x434C29} = 0x7A28180D;
$self->{unk_xor}->{0x436EA2} = 0x6F4B62DA;
$self->{unk_xor}->{0x4373C7} = 0x64973977;
$self->{unk_xor}->{0x4EA772} = 0x6DD14C92;
$self->{unk_xor}->{0x43E3E3} = 0x5E6324D8;
$self->{unk_xor}->{0x442FD5} = 0x62745ED0;
$self->{unk_xor}->{0x44C5AD} = 0x55CD5445;
$self->{unk_xor}->{0x44F3FC} = 0x71DC4898;
$self->{unk_xor}->{0x44F746} = 0x167519CB;
$self->{unk_xor}->{0x44F7CE} = 0x6D3158F8;
$self->{unk_xor}->{0x44F949} = 0x7EA95BEA;
$self->{unk_xor}->{0x45130F} = 0x47377587;
$self->{unk_xor}->{0x451397} = 0x2D8B6E8F;
$self->{unk_xor}->{0x45393C} = 0x65B543C8;
$self->{unk_xor}->{0x4ED511} = 0x4FB96154;
$self->{unk_xor}->{0x456B85} = 0x622C4954;
$self->{unk_xor}->{0x4576FF} = 0x20D220F3;
$self->{unk_xor}->{0x457787} = 0x361D4F0D;
$self->{unk_xor}->{0x459432} = 0x2B2000D1;
$self->{unk_xor}->{0x4594BA} = 0x6FB8593E;
$self->{unk_xor}->{0x45A272} = 0x2B7F7DFC;
$self->{unk_xor}->{0x45F473} = 0x7E221470;
$self->{unk_xor}->{0x46C119} = 0x7A600061;
$self->{unk_xor}->{0x4F7BAB} = 0x43A73788;
$self->{unk_xor}->{0x40A094} = 0x55F4606B;
$self->{unk_xor}->{0x40FA48} = 0x11756B30;
$self->{unk_xor}->{0x412263} = 0x77D64B90;
$self->{unk_xor}->{0x417F7E} = 0x6F872C49;
$self->{unk_xor}->{0x472255} = 0x2E484655;
$self->{unk_xor}->{0x474544} = 0x61640311;
$self->{unk_xor}->{0x47A551} = 0x007044D3;
$self->{unk_xor}->{0x47CDFD} = 0x2D406BB1;
$self->{unk_xor}->{0x47ED68} = 0x351D01ED;
$self->{unk_xor}->{0x4849C1} = 0x26201CA8;
$self->{unk_xor}->{0x41A859} = 0x7AEB3255;
$self->{unk_xor}->{0x494532} = 0x4A1352CF;
$self->{unk_xor}->{0x494CD6} = 0x270C2F52;
$self->{unk_xor}->{0x49B108} = 0x751E6E49;
$self->{unk_xor}->{0x49B530} = 0x670C3F74;
$self->{unk_xor}->{0x49C0EA} = 0x71CD789E;
$self->{unk_xor}->{0x49C17C} = 0x3EB20B7B;
$self->{unk_xor}->{0x4A30C6} = 0x2598043D;
$self->{unk_xor}->{0x4A4E31} = 0x1EED619F;
$self->{unk_xor}->{0x4A5D0A} = 0x6F477561;
$self->{unk_xor}->{0x4A837F} = 0x3B547BFB;
$self->{unk_xor}->{0x4A8983} = 0x2F1467E9;
$self->{unk_xor}->{0x4B3F23} = 0x05FE7AF1;
$self->{unk_xor}->{0x4EB78B} = 0x3C0861B1;
$self->{unk_xor}->{0x4B802A} = 0x7DAB5033;
$self->{unk_xor}->{0x4B94CA} = 0x7F906745;
$self->{unk_xor}->{0x4BA261} = 0x21C46C2C;
$self->{unk_xor}->{0x4BB754} = 0x106B4A85;
$self->{unk_xor}->{0x4BB8EA} = 0x17640F11;
$self->{unk_xor}->{0x4EE9BE} = 0x69E60486;
$self->{unk_xor}->{0x4FB8A5} = 0x6BCA53B0;
$self->{unk_xor}->{0x4F324A} = 0x0B8236E3;
$self->{unk_xor}->{0x4E7B36} = 0x4A2548E8;
$self->{unk_xor}->{0x4DDD87} = 0x24012258;
$self->{unk_xor}->{0x4E4F50} = 0x63CC2377;
$self->{unk_xor}->{0x4E4FE2} = 0x08D04B59;
$self->{unk_xor}->{0x41A0A7} = 0x7C3E0478;
$self->{unk_xor}->{0x4C20CD} = 0x086652BC;
$self->{unk_xor}->{0x4C3765} = 0x6601178E;
$self->{unk_xor}->{0x4C49F0} = 0x07BA21D8;
$self->{unk_xor}->{0x4CBF58} = 0x13AC3D21;
$self->{unk_xor}->{0x4D174E} = 0x3E144154;
$self->{unk_xor}->{0x4D30B6} = 0x3765334C;
$self->{unk_xor}->{0x4D40D7} = 0x16AE5776;
}

1;
